...
\begin{equation}
  ...
  \end{gathered}
\end{equation}
If $F$ satisfies \cref{eq:kbe} for $s\in[0,T]$ and $x\in\Xspace$, then $F(s,x)=\CondExpect{f(\Xr-T)}{\Xr-s=x}$ for all $t\in[0,T]$ and $x\in\Xspace$.
...
Thus, we can define the \emph{saturation} function such that, whenever $P=(T,Z,Y)$ is a pruned genealogy, $s(\leftlim{Y}_t,Y_t)$ is the integer vector of deme-specific numbers of emerging lineages at time $t$.
...
\begin{thm}
  ...
  \begin{equation*}
    \lik=\sum_{y\in\fY_T(\Z)}{\int{w(T,x,y)\,\dd{x}}}.
  \end{equation*}
\end{thm}
...
\begin{corol}
  ...
  Let $V=(\T,\Z)$ be a given obscured genealogy and let $\beta$ and $\varPsi$ be defined as in \cref{thm:obsc-filter}.
  Suppose $F$ is \caglad\ and satisfies the final condition
  $\rightlim{F}(T,x,y)=1$ for all $x\in\Xspace$, $y\in\fY_T(\Z)$ and also the adjoint filter equation
  \begin{align}
...
%%   \end{gathered}
%% \end{equation*}
\Cref{thm:obsc-filter} then states that $\lik=\int{F(0,x)\,p-0(x)\,\dd{x}}$.
...
In this section, we illustrate the theory by working out the filter equations in the case of two familiar models.
...
\begin{mathsize}{9pt}{10pt}
  ...
  \end{equation}
\end{mathsize}%
Again, we now sum over $m$, obtaining
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:sirs3}
    \begin{split}
...
As in \cref{sec:sirs-example}, we sum over $m$, writing $w(t,x,d)=\sum-m{w(t,x,d,m)}$ to obtain
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:seirs22}
    \begin{aligned}
...
\begin{align}
  ...
  \end{split}
\end{align}
One sees, in particular, that the importance-sampling kernels $\pi$ represent the probabilities of changing color on a particular branch (or on no branch), contingent on the population event.
...
With the na\"ive choice of important-sampling kernels, it can happen that the coloring process puts a lineage into a deme that is inconsistent with the data.
...
As we point out in \cref{rem:part-obsc}, a different choice of $\pi$ that anticipates the necessary deme-location, might prove more efficient.
...
Similarly, if any event of $\H$ is incompatible with the local structure of $\P$ in the sense of \cref{sec:compatibility}, then $\CondProb{\Pr-\T=\P}{\Hr-\T=\H}=0$.
...
The $j$-th lineage extends backward in time until it either coalesces with $\P_{j-1}$ or reaches $t=0$ (\cref{fig:embedded-chain}).
...
\begin{lemma}
  ...
  \Cref{eq:filter-eq-defn-reg,eq:filter-eq-defn-sing} are satisfied by $w(t,x)=\int-0^{\infty}{v\,u(t,x,v)\,\dd{v}}$, where $u(t,x,v)$ satisfies the KFE
  \begin{mathsize}{9pt}{10pt}
    \begin{equation}
      \begin{aligned}
