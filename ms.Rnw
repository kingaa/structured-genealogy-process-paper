...
Advent of routine molecular surveillance.
With certain pathogens, with matching of time-scales, information on transmission is possible.
With sufficiently dense sampling, it is sometimes possible to reconstruct WAIFW.
More typically, one wishes to gain insight into the structure and parameterization of the transmission process.
This is accomplished by formalizing a transmission model or suite of transmission models, estimating parameters, and comparing their ability to explain the data.
...
If $\Phir_t$, $t\in\Rp$ is a pure jump process, knowledge of its sample path is equivalent to knowledge of the number, $\Kr_t$, of jumps it has taken as of time $t$, the jump times $\Trh_k$, and the embedded chain $\Phirh_k\coloneq{\Phir_{\Trh_k}}$, $k=0,\dots,\Kr_t$.
...
In particular, let $\leaves$ be a countable set of labels and let $\func{Z}$ be the set of all collections of finite, mutually-disjoint subsets of $\leaves$.
That is, an element $\zeta\in{\func{Z}}$ is a partition of the finite set $\bigcup\zeta\subseteq\leaves$.
...
The tree structure of $G$ is defined by a \cadlag\ map $Z:[0,T]\to\func{Z}$ that is monotone in the sense that $t_1\le{t_2}$ implies $Z_{t_1}\preceq{Z_{t_2}}$.
...
To accommodate all possibilities, let us introduce the indicator function $Q$ such that $Q=1$ if the local genealogy structure---which is captured by the values of $P^{\lab{Y}}$ just before and after $t$---is compatible with an event of type $u$ and $Q=0$ otherwise.
...
\begin{lemma}
  ...
  \begin{equation*}
    \CondExpect{\Vr_t}{\Hr_t=\H_t}=\prod_{e\in\event{\H}_t}{B(e,\Xt_e,\X_e)},
  \end{equation*}
...
\begin{lemma}
  ...
  \begin{equation*}
    \CondExpect{\Vr_t}{\Hr_t=\H_t}=\Indicator{\event{\H}_{t}\supseteq\mathcal{M}_t}\,\prod_{e\in\event{\H}_{t}}{B(e,\Xt_e,\X_e)}.
  \end{equation*}
