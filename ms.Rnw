...
From the mathematical point of view, an essential difficulty in determining expressions for the likelihood of a genealogy generated by a given population process is the fact that a genealogy's structure is determined by events occurring at potentially widely separated times.
...
\begin{equation*}
  ...
  \end{gathered}
\end{equation*}
We will assume that each of the processes treated here is either right-continuous with left limits (\ie \cadlag, so that $\Phir_t=\Philt_t$) or left-continuous with right limits (\caglad, \ie $\Phirt_t=\Phir_t$).
...
For reference, \Cref{tab:symbols} collates the symbols used in the paper with an index to their first appearance in the text.
...
The genealogies depicted in \cref{fig:geneal,fig:upo} pertain to this model and, in \cref{sec:seirs-example}, we work out the theory for the SEIRS model in detail.
...
The tree structure of $G$ is a \caglad\ map $Z:[0,T]\to\partit(\leaves)$ that is monotone in the sense that $t_1\le{t_2}$ implies $Z_{t_1}\preceq{Z_{t_2}}$.
...
Mathematically, if $G=(T,Z,Y)$, then $Y$ is a \caglad\ function that maps each point on the genealogy to a deme and an indicator.
...
Likewise, given $Y$ and $t$, $\col{Y_t}:Z_t\to\Demes$ and $\ctr{Y_t}:Z_t\to\Set{0,1}$ are well defined in the obvious way.
...
In order to apply the general results derived below to specific examples, we will need to be able to speak about events along specific branches of the genealogy, whence we will need additional notation.
In the following, suppose that $z\in\partit(\leaves)$ and $y:z\to\ColorSet$.
For $i\in\Demes$, let $\cols{i}{y}\coloneq\CondSet{b}{y(b)=i}$ be the set of all branches in $y$ having color $i$.

Suppose $a\in\leaves$ is a label, $b\in{z}$ is a branch, $a\in{b}$, and $i,j\in\Demes$.
Let $b'=b\setminus\Set{a}$.
If $b'\ne\emptyset$, we define $\chop{}{ab'}{z}={z\cup\Set{b'}\setminus\Set{b}}$ and,
if $b'=\emptyset$, we let $\chop{}{ab'}{z}={z\setminus\Set{\Set{a}}}$.
...
$\chop{ij}{ab'}{y}(b')=(j,1-\ctr{y}(b))$ and $\chop{ij}{ab'}{y}(c)=y(c)$ for $c\ne{b'}$.
...
Thus, for example, if $(T,Z,Y)$ is a genealogy and there is a sample event at $t$, the label of the sample being $a\in\leaves$, then $\rightlim{Z}_t=\chop{}{ab'}{Z_t}$.
If $Y_t(b)=i$ and it is a terminal sample event, then $\rightlim{Y}_t=\chop{i}{a\emptyset}{Y_t}$;
if it is an inline sample, the descending lineage being in deme $j$, then $\rightlim{Y}_t=\chop{ij}{ab'}{Y_t}$.
...
On branch $b$, the color is swapped from $i$ to $j$ and the indicator is toggled.
That is, $\swap{ij}{b}{y}(b)=\left(j,1-\ctr{y(b)}\right)$ and $\swap{ij}{b}{y}(c)=y(c)$ for all $c\ne{b}$.
...
$\fork{ijk}{cc'}{y}$ to be the coloring of $\fork{}{cc'}{z}$ such that when $y'=\fork{ijk}{cc'}{y}$, then $\col{y'(c)}=j$, $\col{y'(c')}=k$, and $\ctr{y'(c)}=\ctr{y'(c')}=1-\ctr{y(b)}$.
...
It is straightforward to verify that there are well defined operators $\backchop{}{}{}$, $\backswap{}{}{}$, and $\backfork{}{}{}$ such that
\begin{equation}
  \begin{aligned}
...
Let $\ell_i$ denote the number of lineages in deme $i$ just after time $t$ and $\ell\coloneq(\ell_i)_{i\in\Demes}\in\Zp^{\Demes}$.
Clearly, $\ell$ depends only on $\rightlim{Y}_t$.
Therefore, we can define $\ell$ as a function such that, whenever $P=(T,Z,Y)$ is a pruned genealogy, $\ell(\rightlim{Y}_t)$ is the vector of deme-specific lineage counts at time $t$.
...
However, $s$ depends not only on $Y_t$, but also on $\rightlim{Y}_t$.
Thus, we can define the \emph{saturation} function such that, whenever $P=(T,Z,Y)$ is a pruned genealogy, $s(Y_t,\rightlim{Y}_t)$ is the integer vector of deme-specific numbers of emerging lineages at time $t$.
...
To succinctly accommodate all possibilities, let us introduce the indicator function $Q$ such that $Q=1$ if the local genealogy structure---which is captured by the values of $P^{\lab{Y}}$ at and just after $t$---is compatible with an event of type $u$ and $Q=0$ otherwise.
...
we have $\U_t=u$, $\Y_t=y$, and $\Yt_t=y'$.
...
\begin{thm}
  ...
  Then
  \begin{equation*}
    \CondProb{\Pr_\T=\P}{\Hr_\T=\H}=\Indicator{\event{\H}\supseteq{\event{\P}}}\,\prod_{t\in\event{\H}}{\phi^{}_{\U_t}\!(\X_t,\Y_t,\Yt_t)}.
  \end{equation*}
\end{thm}
...
\begin{thm}
  ...
  \begin{equation}
    ...
    \begin{aligned}
      ...
      &\sum_u{\int{w(t,x')\,\alpha_u(t,x',x)\,\phi^{}_u(x,\Y_t,\Yt_t)\,\dd{x'}}}
      -\sum_u{\int{w(t,x)\,\alpha_u(t,x,x')\,\dd{x'}}},
      &t\notin{\event{\P}},\\
      w(t,x)=&\sum_u{\int{\wt(t,x')\,\alpha_u(t,x',x)\,\phi^{}_u(x,\Y_t,\Yt_t)\,\dd{x'}}},
      &t\in{\event{\P}},
    \end{aligned}
  \end{equation}
...
\begin{corol}
  ...
  \begin{equation}
    ...
    \begin{aligned}
      -\frac{\partial{F}}{\partial{s}}(s,x)=
      &\sum_u{\int{\leftlim{\alpha}_u(s,x,x')\,\left[\phi^{}_u(x',\Y_s,\Yt_s)\,F(s,x')-F(s,x)\right]\,\dd{x'}}},
      &s\notin{\event{\P}},\\
      F(s,x)=&\sum_u{\int{\alpha_u(s,x,x')\,\phi^{}_u(x',\Y_s,\Yt_s)\,\rightlim{F}(s,x')\dd{x'}}},
      &s\in{\event{\P}},
    \end{aligned}
  \end{equation}
...
\begin{thm}
  ...
  Then there is a stochastic jump process $\yr_t$ with \caglad\ sample paths in $\fY(\Z)$ such that $(\Xr_t,\Ur_t,\yr_t)$ is Markov and
  \begin{equation*}
    \CondProb{\Pr^{\lab{Z}}_\T=\Z}{\Hr_\T=\H}=
    \Indicator{\event{\H}\supseteq\event{\Z}}\,\Expect{\frac{1}{q(X_0,\yr_0)}\,\prod_{t\in\event{\H}}{\frac{\phi^{}_{\U_t}(\X_t,\yr_t,\yrt_t)}{\pi_{\U_t}(t,\Xt_t,\X_t,\yr_t,\yrt_t)}}},
  \end{equation*}
...
\begin{proof}
  ...
  Here, by definition,
  \begin{equation*}
    \pi(\yr\vert\H)=q(\X_0,\yr_0)\,\prod_{t\in\event{\H}}{\pi_{\U_{t}}(t,\Xt_{t},\X_{t},\yr_{t},\yrt_{t})}.
  \end{equation*}
...
\begin{remark}
  \label{rem:part-obsc}
  Although as defined, an obscured genealogy contains no information about the whereabouts of lineages among demes, \cref{thm:obsc-lik} applies also in the case where one does have information about the deme-location of particular lineages at genealogical event-times.
  For example, it frequently happens that one knows that only certain demes are susceptible to sampling.
  In this case, one chooses the kernel $\pi$ so as to be consistent with known lineage deme-locations.
  Alternatively, one can choose $\pi$ in ignorance of this information, in which case, \cref{thm:obsc-lik} enforces the appropriate penalty for mismatch between the proposed deme-location and that required by the data.
\end{remark}
...
In this section, we illustrate the theory by working out the filter equations in the case of two familiar nonlinear models.
In particular, we show how the general filter equation introduced in \cref{thm:obsc-filter} takes on a specific form that depends only on the model structure.
In each case, we also provide results of some elementary numerical computations based on these filters, using methods described in \cref{sec:filter-eqns}.
...
\begin{mathsize}{9pt}{10pt}
  \begin{equation*}
    \begin{aligned}
      ...
      \quad+\displaystyle\int{\wt(t,x',\backfork{\deme{IEI}}{bb'}{d},\backfork{}{bb'}{m})\,\alpha_{\jump{Trans}}(t,x',x)\,\tbinratio{E}{\ellE}{1}{1}\,\tbinratio{I}{\ellI}{1}{1}\,\dd{x'}}, & t\in{B}.\\
      \end{cases}
    \end{aligned}
  \end{equation*}
\end{mathsize}%
...
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    ...
    \begin{aligned}
      ...
      \quad+\displaystyle\int{\wt(t,x',\backfork{\deme{IEI}}{bb'}{d})\,\alpha_{\jump{Trans}}(t,x',x)\,\tbinratio{E}{\ellE}{1}{1}\,\tbinratio{I}{\ellI}{1}{1}\,\dd{x'}}, & t\in{B}.\\
      \end{cases}
    \end{aligned}
  \end{equation}
\end{mathsize}%
...
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    ...
    \begin{aligned}
      ...
      \displaystyle\int{\wt(t,x',\backfork{\deme{IIE}}{bb'}{d})\,\frac{\pi_{\deme{IE}}^{bb'}(t,x')}{\pi_{\deme{IE}}^{bb'}(t,x')}\,\alpha_{\jump{Trans}}(t,x',x)\,\indicator{I\ge\ellI}\,\indicator{E\ge\ellE}\,\left(\tfrac{1}{EI}\right)\,\dd{x'}}\\[1.5ex]
      \quad+\displaystyle\int{\wt(t,x',\backfork{\deme{IEI}}{bb'}{d})\,\frac{\pi_{\deme{EI}}^{bb'}(t,x')}{\pi_{\deme{EI}}^{bb'}(t,x')}\,\alpha_{\jump{Trans}}(t,x',x)\,\indicator{I\ge\ellI}\,\indicator{E\ge\ellE}\,\left(\tfrac{1}{EI}\right)\,\dd{x'}}, & t\in{B}.\\
      \end{cases}
    \end{aligned}
  \end{equation}
\end{mathsize}%
Here, we have introduced the importance sampling weights $\pi_{\deme{ij}}^{bb'}$, normalized so that $\pi_{\deme{IE}}^{bb'}(t,x)+\pi_{\deme{EI}}^{bb'}(t,x)=1$ for all $t$ and $x$.
...
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    ...
    \begin{split}
      \frac{\partial{w}}{\partial{t}}(t,x,d)=
      &\int{w(t,x',d)\,\alpha_{\jump{Trans}}(t,x',x)\,\frac{\pi_{\deme{IE}}^{\emptyset}(t,x')}{\pi_{\deme{IE}}^{\emptyset}(t,x')}\,\left(\tfrac{E-\ellE}{E}\right)\,\dd{x'}}
      -\int{w(t,x,d)\,\alpha_{\jump{Trans}}(t,x,x')\,\pi_{\deme{IE}}^{\emptyset}(t,x)\,\dd{x'}}\\
      &+\sum_{bd'}\left\{\int{w(t,x',d')\,\alpha_{\jump{Trans}}(t,x',x)\,\frac{\pi_{\deme{IE}}^b(t,x')}{\pi_{\deme{IE}}^b(t,x')}\,\indicator{d=\swap{\deme{IE}}{b}{d'}}\,\left(\tfrac{I-\ellI}{EI}\right)\,\dd{x'}}\right.\\
      &\qquad\qquad\left.-\int{w(t,x,d)\,\alpha_{\jump{Trans}}(t,x,x')\,\pi_{\deme{IE}}^b(t,x)\,\indicator{d'=\swap{\deme{IE}}{b}{d}}\,\dd{x'}}\right\}\\
      &+\int{w(t,x',d)\,\alpha_{\jump{Prog}}(t,x',x)\,\frac{\pi_{\deme{EI}}^{\emptyset}(t,x')}{\pi_{\deme{EI}}^{\emptyset}(t,x')}\,\left(\tfrac{I-\ellI}{I}\right)\,\dd{x'}}
      -\int{w(t,x,d)\,\alpha_{\jump{Prog}}(t,x,x')\,\pi_{\deme{EI}}^{\emptyset}(t,x)\,\dd{x'}}\\
      &+\sum_{bd'}\left\{\int{w(t,x',d')\,\alpha_{\jump{Prog}}(t,x',x)\,\frac{\pi_{\deme{EI}}^b(t,x')}{\pi_{\deme{EI}}^b(t,x')}\,\indicator{d=\swap{\deme{EI}}{b}{d'}}\,\left(\tfrac{1}{I}\right)\,\dd{x'}}\right.\\
      &\qquad\qquad\left.-\int{w(t,x,d)\,\alpha_{\jump{Prog}}(t,x,x')\,\pi_{\deme{EI}}^b(t,x)\,\indicator{d'=\swap{\deme{EI}}{b}{d}}\,\dd{x'}}\right\}\\
...
\begin{equation}
  \begin{gathered}
    \pi_{\deme{IE}}^{\emptyset}(t,x)+\sum_b{\pi_{\deme{IE}}^{b}(t,x)}=1,
    \qquad
    \pi_{\deme{EI}}^{\emptyset}(t,x)+\sum_b{\pi_{\deme{EI}}^{b}(t,x)}=1.
  \end{gathered}
\end{equation}
...
\begin{align}
  \begin{split}
    ...
    \beta(t,x,x',d,d')=&
    \alpha_{\jump{Trans}}(t,x,x')\,\left(\pi_{\deme{IE}}^{\emptyset}(t,x)+\sum_{bd'}{\pi_{\deme{IE}}^b(t,x)\,\indicator{d'=\swap{\deme{IE}}{b}{d}}}\right)\\
    &+\alpha_{\jump{Prog}}(t,x,x')\,\left(\pi_{\deme{EI}}^{\emptyset}(t,x)+\sum_{bd'}{\pi_{\deme{EI}}^b(t,x)\,\indicator{d'=\swap{\deme{EI}}{b}{d}}}\right)\\
    &+\alpha_{\jump{Recov}}(t,x,x')\,\indicator{I>\ellI}+\alpha_{\jump{Wane}}(t,x,x')\\
    ...
    &+\sum_{e\in{B}}{\indicator{b_e\cup{b'_e}\in\cols{\deme{I}}{d}}\,\left[\pi_{\deme{EI}}^{b_eb_e'}(t,x)\,\indicator{d'=\fork{\deme{EI}}{b_eb_e'}{d}}+\pi_{\deme{IE}}^{b_eb_e'}(t,x)\,\indicator{d'=\fork{\deme{IE}}{b_eb_e'}{d}}\right]\,\delta(t,e)}.\\
  \end{split}\\
  \begin{split}
    \label{eq:seirs-boost}
    B_u(t,x,x',d,d')=&
    \indicator{u=\jump{Trans}}\,\left(
    \tfrac{E-\ellE}{E}\,\tfrac{\indicator{d'=d}}{\pi_{\deme{IE}}^{\emptyset}(t,x')}
    +\tfrac{I-\ellI}{EI}\,\tfrac{\indicator{d'=\swap{\deme{IE}}{b}{d}}}{\pi_{\deme{IE}}^b(t,x')}
    \right)\\
    &+\indicator{u=\jump{Prog}}\,\left(
    \tfrac{I-\ellI}{I}\,\tfrac{\indicator{d'=d}}{\pi_{\deme{EI}}^{\emptyset}(t,x')}
    +\tfrac{1}{I}\,\tfrac{\indicator{d'=\swap{\deme{EI}}{b}{d}}}{\pi_{\deme{EI}}^b(t,x')}
    \right)
    +\indicator{u=\jump{Recov}}+\indicator{u=\jump{Wane}}\\
    ...
    &+\indicator{u=\jump{Trans}}\,\sum_{e\in{B}}{\alpha_{\jump{Trans}}(t,x,x')\,\indicator{b_e\cup{b'_e}\in\cols{\deme{I}}{d}}\,\tfrac{1}{EI}\,\left[\tfrac{\indicator{d'=\fork{\deme{EI}}{b_eb_e'}{d}}}{\pi_{\deme{EI}}^{b_eb_e'}(t,x)}+\tfrac{\indicator{d'=\fork{\deme{IE}}{b_eb_e'}{d}}}{\pi_{\deme{IE}}^{b_eb_e'}(t,x)}\right]\,\indicator{t=e}}.\\
  \end{split}\\
  \begin{split}
...
For example, $\pi_{\deme{EI}}^{b}(t,x)$ is the probability of proposing an $\deme{E}\to\deme{I}$ change on branch $b$ at time $t$ given state $x$, given that a $\jump{Prog}$ event occurs at that time.
The complementary probability $\pi_{\deme{EI}}^{\emptyset}(t,x)$ is that of proposing no change to any branch.
As another example $\pi_{\deme{IE}}^{bb'}(t,x)$ is the probability of proposing, at a genealogical branch-point, that branch $b$ is put into deme $\deme{I}$ and $b'$ is put into $\deme{E}$;
$\pi_{\deme{EI}}^{bb'}$ is the probability of the alternative.
One has wide latitude in the choice of these kernels and by tuning them, one can attempt to balance the bias-variance trade-off associated with Monte Carlo integration.
...
Thus, for example, since in the SEIR model, branch points can only occur when the parent is in the $\deme{I}$ deme, one might posit $\pi_{\deme{EI}}$ and $\pi_{\deme{IE}}$ in such a way as to drive the color along a branch into $\deme{I}$ as it approaches a branch point.
...
Thus, in this case, $\pi_{\deme{EI}}^b=\tfrac{1}{E}$ for all $b$ and $\pi_{\deme{EI}}^{\emptyset}=\tfrac{E-\ellE}{E}$.
Similarly, $\pi_{\deme{IE}}^b=\tfrac{1}{I}$ and $\pi_{\deme{IE}}^{\emptyset}=\tfrac{I-\ellI}{I}$.
At branch points, the two possibilities are given equal probabilities, \ie $\pi_{\deme{IE}}^{bb'}=\pi_{\deme{EI}}^{bb'}=\tfrac{1}{2}$.
...
As we point out in \cref{rem:part-obsc}, a different choice of $\pi$ that anticipates the necessary deme-location, might avoid such extreme penalties and thereby prove more efficient.
...
\begin{equation*}
  \CondProb{\Pr_\T=\P}{\Hr_\T=\H}=\prod_{t\in\event{\H}}{\binratio{n(\X_t)}{\ell(\Yt_t)}{r^{\U_t}}{s(\Y_t,\Yt_t)}}.
\end{equation*}
