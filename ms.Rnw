...
If $\Zr$ is such a pure jump process, let $\event{\Zr}{t}=\Set{\Trh_1,\dots,\Trh_{\Kr_t}}$ denote the set of its jump times in the interval $[0,t]$.
...
Wide variety of models are of interest (\cref{fig:examples}).
Linear chain trick.
...
In the following, it will be useful to break the jumps of the population process $\Xr_t$ into distinct categories.
...
\AAK{[Maybe it would be better to speak instead of $\Hr_t$ being the $\sigma$-algebra generated by the sequences $(\Trh_k,\Xrh_k,\Urh_k)_{k=1}^{\Kr_t}$.
...
\begin{figure}
  \begin{center}
    ...
    @
    <<geneal1,dependson="geneal0",fig.dim=c(4,2),out.width="50%">>=
  ...
  \caption{
    ...
    Extant lineages are denoted with black dots;
    blue dots indicate samples.
    \label{fig:geneal}
  }
\end{figure}
...
The $\y$ element of $\G$ specifies the coloring of branches and location of nodes.
...
Let $\Yspace=\Demes\times\Zp$.
For $\eta\in\Yspace$, write $\eta=(\eta^d,\eta^m)$ to denote the two components.
Then, for $\zeta\in\func{Z}$, each $\eta\in\Yspace^{\zeta}$ is a map from $\zeta$ into $\Yspace$.
Now, given a tree $\z:[0,T]\to\func{Z}$ and a map $\y:[0,T]\to{\coprod_{t\in[0,T]}{\Yspace^{\z_t}}}$,
we say that $\y$ is \emph{compatible} with $\z$ if,
for every $t\in[0,T]$, $\y_t\in{\Yspace^{\z_t}}$,
$\y$ is \cadlag\ and,
whenever $t_1,t_2\in[0,T]$, $b_1\in\z_{t_1}$, $b_2\in\z_{t_2}$
such that $t_1\le{t_2}$ and $b_1\supseteq{b_2}$,
we have $y^m_{t_1}\le{y^m_{t_2}}$.
Let $\func{Y}(\z)$ denote the set of such maps compatible with $\z$ and $\func{Y}_t(\z)\coloneq\CondSet{y_t}{y\in\func{Y}(\z)}$.
\AAK{[The coproduct is a fiber bundle over the space of trees and $\func{Y}(\z)$ is a section of this bundle.]}
...
\begin{figure}
  <<geneal2,dependson="geneal0",fig.dim=c(4,6),out.width="50%">>=
  pal <- c("#00274CFF","#FFCB05FF")
  plot_grid(
    A=x |>
      plot(
