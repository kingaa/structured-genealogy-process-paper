...
We use bold-face to mark random elements and the corresponding regular-face symbol to denote specific values of such elements.
Given any \cadlag\ process $f_t$, the set of jump-times is denoted $\event(f_t)=\{e\in[0,t]\;\vert\;\leftlim{f}_e\ne{f_e}\}$.
...
\begin{thm}\label{thm:pruned_lik}
  ...
  \begin{equation*}
    \begin{aligned}
      \Prob{P^*\vert\Hist_t}=&\prod_{e\in\event(\Hist_t)}{\BinRatio{n(X_e)}{\ell(y^*(e))}{r^{U_e}}{s(\yt^*(e),y^*(e))}\,Q_{U_e}(\yt^*(e),y^*(e))}\\
      &\qquad\times\Indicator{\event(P^*)\subseteq{\event(\Hist_t)}}.
    \end{aligned}
  \end{equation*}
\end{thm}
\begin{proof}
  As we have already observed, if $\event(P^*)\nsubseteq\event(\Hist_t)$, then $\Prob{P^*\vert\Hist_t}=0$.
  Similarly, if there is any event of $P^*$ which is incompatible with the population process, $\Prob{P^*\vert\Hist_t}=0$.
  ...
  \end{equation*}
  Because our lineages are labelled, each of the $\binom{\ell_i}{s_i}$ equally probable sets of $s_i$ lineages is distinct; just one of these is the one present in $P^*$.
...
\begin{thm}\label{thm:obsc_lik}
  ...
  Let $\pi:\Jumps\times\Rp\times\Xspace^2\times\Yspace^2\to\Rp$ be a probability kernel, \ie $\pi_u(t,\xi,\xi',\eta,\eta')\ge{0}$ and $\sum_{\eta'}{\pi_u(t,\xi,\xi',\eta,\eta')}=1$.
  Suppose moreover that $\pi_u(t,\xi,\xi',\eta,\eta')>0$ if and only if $Q_u(\eta,\eta')=1$.
  Let $\rdm{y}_t$ be the Markov jump process on $\Yspace$ generated by the kernel $\pi$, with $\event(\rdm{y})=\event(\Hist_t)$.
  Then
  \begin{equation*}
    \Prob{V^*\vert\Hist_t}=\Expect{\left.\prod_{e\in\event(\Hist_t)}{\frac{\scriptsize\BinRatio{n(X_e)}{\ell(\rdm{y}_e)}{r^{U_e}}{s(\leftlim{\rdm{y}}_e,\rdm{y}_e)}}{\scriptsize\pi_{U_e}(e,\leftlim{X}_e,X_e,\leftlim{\rdm{y}}_e,\rdm{y}_e)}}\times\Indicator{\obs(P(\rdm{y}))=V^*}\;\right\vert\;\Hist_t},
  \end{equation*}
...
\begin{proof}
  ...
  \end{equation*}
  Now let $\event(\Hist_t)=\{t_1,t_2,\dots,t_K\}$.
  $\rdm{\hat{y}}_k$ is the embedded chain of a unique, piecewise-constant, \cadlag\ process $\rdm{y}_t$, with jump times at $t_k$.
  The conditions on $\pi$ guarantee that, for every pruned genealogy $P$, $Y^P$ is a sample path of this process.
  In particular, they guarantee that $\pi_u(t,\xi,\xi',\eta,\eta')\;\propto\;{Q_u(\eta,\eta')}$.
  Clearly, the construction of $\rdm{y}_t$ guarantees that $\event(P(\rdm{y}))\subseteq\event(\Hist_t)$.

  We therefore have that
  \begin{equation*}
    \Prob{V^*\vert\Hist_t}=\Expect{\frac{\Prob{P(\rdm{y})\vert\Hist_t}}{\pi(\rdm{y}\vert\Hist_t)}\cdot\Indicator{\obs(P(\rdm{y}))=V^*}},
  \end{equation*}
...
\Cref{eq:filterlemma} is recognizable as the KFE of a certain process $(X_t,V_t)$.
In particular, $X_t$ is the driver with KFE \cref{eq:kfe2}.
The $V_t$ process has jumps wherever $X_t$ does, such that when $X$ jumps at time $t$ from $x$ to $x'$, $V$ jumps by the multiplicative factor $B(t,x,x')>0$.
Between jumps, $V_t$ decays deterministically and exponentially at rate $\lambda(t,x)$.
If we view $V_t$ as a weight, then \cref{lemma:filtereq} tells us how the $V_t$-weighted average of $X_t$ evolves in time:
...
\begin{prop}\label{prop:zeroboost}
  ...
  %% Suppose that $B(t,x,x')>0$, $Q(t,x,x')\in\{0,1\}$, and $\lambda(t,x)\in\R$ are given measurable functions.
  %% Consider the equation
  %% \begin{equation*}
  %%   \frac{\partial{w}}{\partial{t}}
  %%   =\int{w(t,x')\,\beta(t,x',x)\,B(t,x',x)\,Q(t,x',x)\,\dd{x'}}
  %%   -\int{w(t,x)\,\beta(t,x,x')\,\dd{x'}}
  %%   -\lambda(t,x)\,w(t,x)
  %% \end{equation*}
  Let $\Hist_t$ be its history process.
  Suppose $\rdm{R}_t$ is a real-valued random process such that
  \begin{equation*}
    \Expect{\rdm{R}_t\;\vert\;\Hist_t}=\prod_{e\in\event(\Hist_t)}{Q(e,\leftlim{X}_e,X_e)\,B(e,\leftlim{X}_e,X_e)},
  \end{equation*}
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  Then $\Expect{\rdm{R}_t}=\int{w(t,x)\dd{x}}$.
\end{prop}
\begin{proof}
...
\begin{prop}
  ...
  \end{equation*}
  Let $\Hist_t$ be its history process.
  Suppose that $0<t_1<\dots<t_N\le{t}$ are fixed times and set $M=\{t_1,\dots,t_N\}$.
  Let $B>{0}$ and $Q\in\{0,1\}$ be given measurable functions.
  Let $\rdm{R}$ be a non-negative real-valued random variable such that
  \begin{equation*}
    \Expect{\rdm{R}\;\vert\;\Hist_t}=\prod_{e\in{\event(\Hist_t)}}{Q(e,\leftlim{X}_e,X_e)\,B(e,\leftlim{X}_e,X_e)}.
  \end{equation*}
  Let $\pi(t,x,x')$ be a given probability kernel and $\Y_t$ be the Markov process generated by \cref{eq:kfe2} where $\beta(t,x,x')=\left[\alpha(t,x,x')+\sum_{e\in{M}}{\delta(t,e)\,\pi(t,x,x')}\right]\,Q(t,x,x')$.
  Suppose $w$ satisfies the filter equation with driver $\Y_t$, boost
  \begin{equation*}
    C(t,x,x')=
    \begin{cases}
      B(t,x,x'), &t\notin{M},\\
      B(t,x,x')\,\displaystyle\frac{\alpha(t,x,x')}{\pi(t,x,x')}, &t\in{M},
    \end{cases}
  \end{equation*}
  and decay $\lambda=\int{\alpha(t,x,x')\,\left(1-Q(t,x,x')\right)\,\dd{x'}}$.
  Then
  \begin{equation*}
    \Expect{\rdm{R}\;\big\vert\;M\subseteq{\event(\Hist_t)}}=\int{w(t,x)\,\dd{x}}.
  \end{equation*}
\end{prop}
\begin{proof}
...
\begin{equation*}
  ...
  \end{gathered}
\end{equation*}
We can always take a version of $\X_t$ with \cadlag\ sample paths.

Let $\rdm{\hat{X}}_k$ be the embedded chain of $\X_t$ and let $\rdm{T}^X_k$ be the point process of its jump times.
It is elementary that
\begin{equation*}
  \Prob{\rdm{\hat{X}}_k=\hat{X}_k\;\Big\vert\;\rdm{\hat{X}}_{k-1}=\hat{X}_{k-1},\rdm{T}^X_k=T_k}=\frac{\alpha(T_k,\hat{X}_{k-1},\hat{X}_k)}{\int{\alpha(T_k,\hat{X}_{k-1},x')\dd{x'}}}.
\end{equation*}
Moreover, if one has the $(\rdm{\hat{X}}_k,\rdm{T}^X_k)$ chain, one can reconstruct the $\X_t$ process.
Conditional on $\rdm{\hat{X}}$, the jump-time chain $\rdm{T}^X_k$ is a Poisson point process with intensity $\int{\alpha(t,\X_t,x')\dd{x'}}$.

We can rewrite the KFE as
\begin{equation}\label{eq:poissdriver}
  \begin{aligned}
    \frac{\partial{w}}{\partial{t}}(t,n,x)=&\int\!w(t,n-1,x')\,\nu\,\pi(t,x',x)\,B(t,x,x')\,\dd{x'}-\int\!w(t,n,x)\,\alpha(t,x,x')\,\dd{x'}\\
    =&\int\!w(t,n-1,x')\,\nu\,\pi(t,x',x)\,B(t,x,x')\dd{x'}-\int\!w(t,n,x)\,\nu\,\pi(t,x,x')\,\dd{x'}\\
    &\qquad-\lambda(t,x)\,w(t,n,x)
  \end{aligned}
\end{equation}
where $\nu>0$ is the intensity of homogenous Poisson process and
\begin{equation*}
  \begin{gathered}
  ...
  \end{gathered}
\end{equation*}
Note that $\pi$ is the probability kernel of the embedded chain $\rdm{\hat{X}}$ and $A(t,x)$ is the intensity of the $\rdm{T}^X_k$ process.
We recognize this equation as the filter equation with the rate-$\nu$ Poisson-process driver, boost $B$, and decay $\lambda$.

By integrating equations of this kind, we effectively integrate over the $(\rdm{\hat{X}},\rdm{T}^X)$ process.
To marginalize over only some of the $\rdm{T}^X_k$, we reason as follows.
Fix $0<t_1<\dots<t_K\le{T}$ and $\nu>0$.
Consider the mixed point process $T_k$ with intensity $\nu+\sum_{k=1}^K\delta(t,t_k)$.
Using this as the driver in \cref{eq:poissdriver}, we obtain
\begin{equation*}
  \begin{aligned}
    \frac{\partial{w}}{\partial{t}}(t,n,x)=&\int{w(t,n-1,x')\,\nu\,\pi(t,x',x)\,B(t,x',x)\dd{x'}}-\int{w(t,n,x)\,\nu\,\pi(t,x,x')\,\dd{x'}}\\
    &\quad+\sum_{k}\delta(t,t_k)\,\left\{\int{w(t,n-1,x')\,\pi(t,x',x)\,B(t,x,x')\dd{x'}}-\int{w(t,n,x)\,\pi(t,x,x')\,\dd{x'}}\right\}\\
    &\qquad-\lambda(t,x)\,w(t,n,x).
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
    \frac{\partial{w}}{\partial{t}}(t,n,x)=&\int{w(t,n-1,x')\,\nu\,\pi(t,x',x)\,B(t,x',x)\dd{x'}}-\int{w(t,n,x)\,\nu\,\pi(t,x,x')\,\dd{x'}}\\
    &\qquad-\lambda(t,x)\,w(t,n,x),\\
    w(t_k,n,x)=&\int{\leftlim{w}(t_k,n-1,x')\,\pi(t_k,x',x)\,B(t_k,x',x)\,\dd{x'}}\\
    &\qquad=\tfrac{1}{\nu}\,\int{\leftlim{w}(t_k,n-1,x')\,\alpha(t,x',x)\,\dd{x'}}\\.
  \end{aligned}
\end{equation*}
We then have $\Prob{\X_t\in\mathcal{E}\;\big\vert\;t_1,\dots,t_K\in\event(\Hist_t)}=\int_{\mathcal{E}}{w(t,x)\,\dd{x}}$.

Now suppose $\rdm{R}_k$ is a Markov chain satisfying
\begin{equation*}
  \Prob{\rdm{R}_k=R_{k-1}\,B(T_k,\hat{X}_{k-1},\hat{X}_k)\;\Big\vert\;\rdm{\hat{X}}_k=\hat{X}_k,\rdm{\hat{X}}_{k-1}=\hat{X}_{k-1},\rdm{T}^X_k=T_k,\rdm{R}_{k-1}=R_{k-1}}=1,
\end{equation*}
for some function $B>0$.
\AAK{Or perhaps the following is sufficient for our purposes?}
\begin{equation*}
  \Expect{\rdm{R}_k\;\Big\vert\;\rdm{\hat{X}}_k=\hat{X}_k,\rdm{\hat{X}}_{k-1}=\hat{X}_{k-1},\rdm{T}^X_k=T_k,\rdm{R}_{k-1}=R_{k-1}}=R_{k-1}\,B(T_k,\hat{X}_{k-1},\hat{X}_k).
\end{equation*}
Let $\rdm{R}_t$ be the corresponding continuous-time jump process with jumps at $\rdm{T}^X_k$.
By \cref{lemma:filtereq}, if $w$ satisfies the boost equation with boost $B$, driver $\X$, and decay $\lambda=0$, then
\begin{equation*}
  \Expect{\rdm{R}_t}=\int{w(t,x)\dd{x}}.
\end{equation*}
