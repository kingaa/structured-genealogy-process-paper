...
In particular, if $M$ represents a dynamic transmission model, $S$ a set of genome sequences, $\Phi$ a genealogical tree relating these sequences, and $E$ a model of sequence evolution along a tree,
then the likelihood of $S$ is a function
\begin{equation*}
  f(S|M,E) = \int{f(S|\Phi,E)\,f(\Phi|M)\,\dd{\Phi}},
\end{equation*}
where we somewhat loosely use the functions $f$ to refer to distinct probability densities, the nature of each of which is clear from its arguments and the integral is taken over all possible genealogies relating the sequences $S$.
In this expression, $f(S|\Phi,E)$ is the \citet{Felsenstein2004} likelihood.
The function $f(\Phi|M)$, which links the phylogeny $\Phi$ to the dynamic model $M$, may be termed the \emph{phylodynamic likelihood}.
...
Extensions of this approach develop approximate likelihoods for the case when the population size varies slowly \citep{Drummond2005} or when the sample fraction is negligible \citep{Volz2009a}.
...
Although the tractability of these approaches are attractive, concern naturally arises as to validity of the approximations, the biases introduced by them, and the amount of information in data left unutilized by these approximate methods.

%% Factorization of problem into two subproblems.
%% Problems with this approach \citep{Smith2017}.

What would an improved phylodynamic inference approach look like?
...
Second, because nonlinearity, noise, and measurement error are so prominent and ubiquitous in epidemiology, it would accommodate nonlinear, stochastic transmission models.
...
For example, in a compartmental model, one can assume that individual hosts within a compartment are exchangeable.
This implies that their residence times within a compartment are identically distributed, though not independent.
Although this is indeed an additional assumption, it is so natural that it is frequently unrecognized as such, and one often reads statements to the effect that exchangeability of individuals is a consequence of the Markovian assumption about the discretely structured population.
Nonetheless, since it adds minimum additional structure, it is the natural assumption, and the one we will make in this paper.
...
In particular, one can model not only complex infection progression, but also strain structure, behavioral structure (\eg superspreading), age structure, and spatial structure using compartmental models.
...
\begin{equation*}
  ...
  \end{gathered}
\end{equation*}
We further assume that $\alpha(t,x,x')$ is \cadlag\ as a function of time for all $x,x'\in{\Xspace}$ and that the number of jumps that occur in a finite time-interval is finite: $\Prob{\Kr_t<\infty}=1$ for all $t$.
...
For modeling purposes, it is sometimes desirable to insist that certain events occur at specific, known times.
...
Using a Dirac delta notation, one can represent \cref{eq:kfe-reg,eq:kfe-sing} as a single equation in the form of \cref{eq:kfe}.
...
Here, $\dd{\pi}/\dd{x}$ denotes the density (\ie Radon-Nikodym derivative) of $\pi$ with respect to the measure on $\Xspace$.
...
It will be useful to divide the jumps of the population process $\Xr_t$ into distinct categories.
...
Demes can be used to introduce spatial, behavioral, physiological, and life-history structure into a model.
The flexibility this allows can also be used, via the linear chain trick, to introduce delays into the dynamics.
...
Moreover, by construction, the lineages within each deme are exchangeable:
...
\Cref{lemma:filter} shows how this can be computed via Sequential Monte Carlo.
...
This approach gives us the freedom to choose models with many demes.
Moreover, there is freedom to choose the sampling model.
...
The price for this flexibility is variability in the Monte Carlo estimation.
However, there is substantial freedom in the choice of the importance sampling distribution $\pi$.
Moreover, since it is permissible to borrow information from the future, one can envision importance sampling distributions tailored to any given genealogy.
...
Relations to filtering of continuous-time Markov chains by \citet{Giesecke2018}.
...
\begin{proof}
  ...
  \AAK{[This depends on $\alpha$ and $B$ being continuous in their first arguments.]}
\end{proof}
...
\begin{lemma}
  Showing how \cref{eq:two-part-filter-reg,eq:two-part-filter-sing} can be stated as a singular filter equation, \ie with driver $\beta$ and boost $C$, where
  \begin{equation*}
    \begin{gathered}
...
\begin{lemma}
  \label{lemma:filter}
  The filter equation \eqref{eq:reg-filter-eq-reg} is satisfied by $w(t,x)=\int_0^{\infty}{v\,u(t,x,v)\,\dd{v}}$, where $u(t,x,v)$ satisfies the KFE
  \begin{equation}
...
\begin{mathsize}{10pt}{11pt}
  \begin{equation*}
    \begin{split}
      \frac{\partial{v}}{\partial{t}}(t,S,I,R)=
      &\frac{\beta(t)\,(S+1)\,(I-1)}{N}\,v(t,S+1,I-1,R)
      -\frac{\beta(t)\,S\,I}{N}\,v(t,S,I,R)\\
...
\begin{mathsize}{10pt}{11pt}
  \begin{equation*}
    \begin{split}
      ...
      &\frac{\beta(t)\,(S+1)\,I}{N}\,v(t,S+1,E-1,I,R)
      -\frac{\beta(t)\,S\,I}{N}\,v(t,S,E,I,R)\\
...
\subsection{Linear birth-death model}
...
We recognize \cref{eq:kingman} as the expression for the \citet{Kingman1982a} coalescent \citep[\eg][]{Wakeley2008}.
