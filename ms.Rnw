...
Numerical examples are postponed to \cref{sec:worked-examples}, and computational scalability is discussed in \cref{sec:discussion}.
...
At the end of the main text, \cref{tab:symbols} is an index of the mathematical notation used in the paper.
...
A complete index to the symbols and acronyms used in the paper is to be found in \cref{tab:symbols}.
...
\Cref{fig:example-models} depicts a few of the simplest such models in order to give a sense of the kinds of complexities that can arise.
...
\begin{figure}
  ...
  \caption{
    ...
    Markov state transition diagram for the SEIRS model depicted in \cref{fig:example-models}A.
    ...
    In the terminology of \cref{sec:event-types}, each of these is of a different type:
...
\end{equation*}
\Cref{fig:markov-state} shows an example for which $\Jumps$ has five elements.
...
\Cref{fig:example-models} illustrates this concept in the context of several specific models.
...
In \cref{sec:sirs-example}, we illustrate the theory by working it out in detail for this model.

\paragraph{SEIRS model}

Modifying the SIRS model by incorporating an incubation period results in the SEIRS model, which has two demes (\cref{fig:example-models}A):
...
The genealogies depicted in \cref{fig:geneal,fig:upo} pertain to this model and, in \cref{sec:seirs-example}, we work out the theory for the SEIRS model in detail and display results of some numerical computations.

\paragraph{Two-strain competition model}

A simple model for the competition of two pathogen strains is depicted in \cref{fig:example-models}B.
...
\Cref{fig:example-models}C depicts a model in which infections are heterogeneous with respect to the severity of the disease they engender.
...
\Cref{fig:example-models}D depicts a model with behavioral heterogeneity.
...
\end{compactenum}
\Cref{fig:markov-state} depicts an example with jumps of all five pure types.
...
At each jump in the population process, a change is made to the genealogy, according to the mark, $u$, of the jump (\cref{fig:event-types}).
In particular:
\begin{compactenum}[(a)]
\item
  If $u$ is of birth-type (\cref{fig:event-types}A), it results in the creation of one new internal node, call it $b$.
...
\item
  If $u$ is of death-type (\cref{fig:event-types}B), one or more tip nodes of the appropriate demes are selected with uniform probability from among those present.
  These are deleted.
  Next, non-sample internal nodes without children are recursively removed.
  Sample nodes are never removed.
\item
  At a migration-type event (\cref{fig:event-types}C), the appropriate number of migrating lineages are selected at random with uniform probability, from among those present in the appropriate deme.
...
\item
  At a sample-type event (\cref{fig:event-types}D), the appropriate number of sampled lineages are selected at random from among the tip nodes, with uniform probability according to deme.
  One new sample node is introduced for each selected lineage:
  each is inserted between a selected tip node and its ancestor.
\item
  At a neutral-type event (\cref{fig:event-types}E), no change is made to the genealogy.
\item
  Finally, events of compound type (\eg \cref{fig:event-types}F--H) are accommodated by combining the foregoing rules.
\end{compactenum}
...
\begin{figure}
  ...
  \caption{
    \label{fig:upo}
    Unpruned, pruned, and obscured genealogies from a single realization of the genealogy process induced by the SEIRS model depicted in \cref{fig:example-models,fig:markov-state}.
...
In the following, \cref{thm:pruned-lik} establishes the likelihood of a given pruned genealogy conditional on the history of the population process.
\Cref{thm:pruned-filter} then shows how the history can be integrated out.
\Cref{thm:obsc-lik} gives the likelihood of an obscured genealogy conditional on the population history via an auxiliary coloring process.
The main result, \cref{thm:obsc-filter} integrates out the history and coloring processes jointly to obtain the likelihood.
\Cref{corol:pruned-filter,corol:obsc-filter} provide the adjoint (backward) formulations of the filter equations derived in \cref{thm:pruned-filter,thm:obsc-filter}, respectively.
In \cref{sec:special-cases}, we demonstrate that the closed-form likelihood formulas for the \citet{Kingman1982a} coalescent and linear birth-death-sampling processes fall out as special cases of these results.
...
\begin{thm}
  ...
  \end{equation}
  where $n$ is the deme occupancy (\cref{sec:demes}), $r^u$ is the production (\cref{sec:production}), $\ell$ and $s$ are the lineage-count and saturation functions, respectively (\cref{sec:ells}), $Q$ is the compatibility indicator (\cref{sec:compatibility}), and the binomial ratio is as defined in \cref{sec:binomial-ratio}.
  Then
  \begin{equation*}
...
For this, we use the filter equation technology developed in \cref{sec:filter-eqns}.
In particular, the following theorem follows immediately from \cref{thm:pruned-lik,lemma:sing-filt}.
...
\begin{remark}
  ...
  The quantity $w(t,x)$ appearing in \cref{eq:pruned-filter-eqn} is not itself obviously a probability, except at $t=T$.
  ...
  Specifically, $w(t,x)$ represents an expectation of partial versions of the products appearing in \cref{thm:pruned-lik} (containing only the factors for times $<t$) over population trajectories having terminal value $x$ at time $t$.
\end{remark}

One can alternatively express the likelihood of a pruned genealogy using the adjoint form of the filter equation (see \cref{sec:adjoint-filter}).

\begin{corol}
  \label[corol]{corol:pruned-filter}
...
\begin{proof}
  ...
  \end{equation*}
  The result then follows from \cref{thm:pruned-lik}.
\end{proof}
...
It is an immediate consequence of \cref{thm:obsc-lik,lemma:sing-filt}.

\begin{thm}
  \label{thm:obsc-filter}
  Let $(\T,\Z)$ be a given obscured genealogy.
  Then there are probability kernels $q$ and $\pi$ as in \cref{thm:obsc-lik} such that if
  \begin{equation*}
    \begin{gathered}
...
\Cref{lemma:monte-carlo} shows how the likelihood can be computed via sequential Monte Carlo.
As before, there is also an adjoint form of \cref{thm:obsc-filter}, to wit:

\begin{corol}
  \label[corol]{corol:obsc-filter}
  Let $(\T,\Z)$ be a given obscured genealogy and let $\beta$ and $\varPsi$ be defined as in \cref{thm:obsc-filter}.
...
At each event, a compound birth-death jump occurs so that the population size, $n$, remains constant (cf.~\cref{fig:event-types}F).
...
Since nothing depends on the state, in writing the corresponding filter equation (\cref{eq:obsc-filter-reg}), we can sum over both $x$ and $y$.
Then, if $B$ is the set of branch-times and $\ell(t)$ is the number of lineages in the genealogy at time $t$, upon this summation, \cref{eq:obsc-filter-reg,eq:obsc-filter-sing} become
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
...
More generally, if in addition samples are taken according to a rate-$\psi$ Poisson process such that the set of sample-times in the genealogy is $S=S_0\cup{S_1}$, where $S_0$, $S_1$ are the sets of times of terminal and inline samples, respectively, then \cref{eq:obsc-filter-reg,eq:obsc-filter-sing} reduce to
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
...
\Cref{eq:obsc-adj-reg} for this process reads
\begin{mathsize}{9pt}{10pt}
  \begin{equation*}
...
We then sum over $y$ and apply the Chu-Vandermonde identity (\cref{eq:chu-vandermonde}) to obtain
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
...
The singular part (\cref{eq:obsc-adj-sing}) reads
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
...
\begin{equation}
  ...
  \end{gathered}
\end{equation}
In passing, we note that \cref{eq:FGeq} appear prominently in the very different derivation of \citet{Stadler2010}.
...
%%   \end{gathered}
%% \end{equation*}
\Cref{thm:obsc-filter} then states that $\lik=\int{F(0,x)\,p_0(x)\,\dd{x}}$.
...
In particular, we will show how the general filter equation introduced in \cref{thm:obsc-filter} takes on a specific form that depends only on the model structure.
In each case, we will also provide results of some elementary numerical computations based on these filters, using methods described in \cref{sec:filter-eqns}.
...
However, the interested reader may find it instructive to see how the general expressions of \cref{thm:obsc-filter} are translated into concrete equations relative to a particular model.
...
\begin{figure}
  ...
  \caption{
    ...
    The branch immediately ancestral to $a$ is therefore $b\cup\Set{a}$.
...
Recall from \cref{sec:genealogy} the formal definition of a pruned genealogy as a triple $(\T,\Z,\Y)$ where $\T$ is the genealogy time; $\Z$, the tree structure; and $\Y$ the coloring.
...
Despite its relative simplicity, it will be instructive to consider, as a first illustration of the application of the theory, the SIRS model discussed in \cref{sec:examples}.
Let us deduce the filter equation (\cref{eq:obsc-filter-reg,eq:obsc-filter-sing}) corresponding to this one-deme model.
...
\Cref{tab:sirs-model-elements} gives $\alpha_u$, $r^u$, and the event type for each of these marks.
...
\begin{mathsize}{9pt}{10pt}
  ...
  \end{equation}
\end{mathsize}%
(Recall that the swap, chop, and fork operators $\swap{}{}{}$, $\chop{}{}{}$, $\fork{}{}{}$ are defined in \cref{sec:swapnchop}).
...
\begin{table}
  \caption{
    ...
    For each of these, the table shows the rate ($\alpha_u$), production ($r^u$), and event type (\cf~\cref{sec:event-types}).
  }
...
The general form of the singular part of the filter equation (\cref{eq:obsc-filter-sing}) is
\begin{equation*}
  w(t,x,y)=\sum_{uy'}{\int{\wt(t,x',y')\,\alpha_u(t,x',x)\,\phi_u(t,x',x,y',y)\,\dd{x'}}},\qquad t\in\event{Z}.
\end{equation*}
Recalling that a coloring has two components, \ie $y=(d,m)$, where $d=\col{y}$ is the deme, or color, and $m=\ctr{y}$ is the event indicator (\cf~\cref{sec:genealogy}), we can make these explicit in writing $w=w(t,x,d,m)$.
...
Applying the relevant cases of \cref{eq:sirs-Qs} (\ie the first three), and using \cref{eq:phidef}, we can rewrite \cref{eq:sirs5} as
\begin{equation*}
  w(t,x,m)=\begin{cases}
...
\begin{equation}
  ...
  \end{cases}\qquad
\end{equation}
Finally, putting in the expressions from \cref{eq:sirs-binratio,tab:sirs-model-elements} gives
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:sirs7}
    w(t,S,I,R)=
...
Let us now turn to the regular part of the filter equation (\cref{eq:obsc-filter-reg}), which holds for $t\notin{B\cup{S_0}\cup{S_1}}$,
...
\begin{mathsize}{9pt}{10pt}
  ...
  \end{equation}
\end{mathsize}%
Using \cref{eq:phidef,eq:sirs-binratio,eq:sirs-Qs}, we can evaluate the sums in \cref{eq:sirs1} to obtain
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:sirs2}
    \begin{split}
...
\begin{mathsize}{9pt}{10pt}
  ...
  \end{equation}
\end{mathsize}%
Finally, inserting the expressions from \cref{tab:sirs-model-elements} and \cref{eq:sirs-binratio} leaves us with the following regular filter equation
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:sirs4}
    \begin{split}
...
For $I<\ell$, it follows from \cref{eq:sirs7} that $w=0$.
Here we have used the Chu-Vandermonde identity (\cref{eq:chu-vandermonde}) to rewrite the first term of \cref{eq:sirs3}.
Note the presence in \cref{eq:sirs4} of the decay term proportional to $\psi$.

\Cref{eq:sirs4,eq:sirs7} are identical to those obtained using the more specialized approach of \citet{King2022}.
\Cref{fig:sirs-example} shows some numerical results obtained by integrating this filter equation using the scheme of \cref{sec:smc}.
...
A simple model with more than one deme is the SEIRS model (\cref{fig:example-models}A, \cref{fig:markov-state}).
...
\Cref{tab:seirs-model-elements} gives $\alpha_u$, $r^u$, and the event type for each of these marks.
The relevant binomial ratios are
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:seirs1}
...
Now, writing $d=\col{y}$ and $m=\ctr{y}$ so that $y=(d,m)$ and $w=w(t,x,d,m)$, the singular portion of the filter equation (\cref{eq:obsc-filter-reg}) has general form
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:seirs21}
  ...
  \end{equation}
\end{mathsize}%
Applying \cref{eq:phidef} and the relevant cases of \cref{eq:seirs1,eq:seirs2}, we expand the sums to obtain
\begin{mathsize}{9pt}{10pt}
  \begin{equation*}
    \begin{aligned}
  ...
  \end{equation*}
\end{mathsize}%
Note that, in \cref{eq:seirs21}, the sample label $a$, and the branches $b$ and $b'$ are determined unambiguously by $Z$.
As in \cref{sec:sirs-example}, we sum over $m$, writing $w(t,x,d)=\sum_m{w(t,x,d,m)}$ to obtain
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:seirs22}
    \begin{aligned}
  ...
  \end{equation}
\end{mathsize}%
Finally, substituting the expressions from \cref{eq:seirs1}, we have the following singular equation:
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:seirs23}
    \begin{aligned}
...
Turning now to the filter equation's regular part, \cref{eq:obsc-filter-reg} is equivalent to
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:seirs3}
    \begin{split}
  ...
  \end{equation}
\end{mathsize}%
Substituting the expressions from \cref{eq:phidef,eq:seirs1,eq:seirs2} into \cref{eq:seirs3} and evaluating the sums as in \cref{sec:sirs-example} yields
\begin{mathsize}{9pt}{10pt}
  \begin{equation*}
    \label{eq:seirs4}
    \begin{split}
  ...
  \end{equation*}
\end{mathsize}%
As in \cref{sec:sirs-example}, we then sum over $m$, obtaining
\begin{mathsize}{9pt}{10pt}
  \begin{equation*}
    \begin{split}
  ...
  \end{equation*}
\end{mathsize}%
Simplifying using \cref{eq:seirs1,eq:chu-vandermonde,eq:swapnchop} and splitting the final sum over $u$, we obtain, for $E\ge{\ellE}$, $I\ge{\ellE}$,
\begin{mathsize}{9pt}{10pt}
  \begin{equation}
    \label{eq:seirs5}
    \begin{split}
...
The kernels $\pi$ that appear in \cref{eq:seirs5} are normalized so that, for all $t,x$,
\begin{equation}
  \begin{gathered}
...
Comparing \cref{eq:seirs5,eq:seirs21} with the general form of a filter equation (\cref{sec:filter-eqns}, \cref{eq:filter-eq-defn-reg,eq:filter-eq-defn-sing}), one recognizes them as a filter equation on the $(x,d)$-space, with driver $\beta$, boost $B$, and decay $\lambda$, as follows.
\begin{align}
  \begin{split}
    \label{eq:seirs-driver}
    \beta(t,x,&x',d,d')=
    \alpha_{\jump{Trans}}(t,x,x')\,\left(\pi_{\jump{Trans}}^{\emptyset}(t,x)\,\indicator{d'=d}+\sum_{b}{\pi_{\jump{Trans}}^b(t,x)\,\indicator{d'=\swap{\deme{IE}}{b}{d}}}\right)\\
    &+\alpha_{\jump{Prog}}(t,x,x')\,\left(\pi_{\jump{Prog}}^{\emptyset}(t,x)\,\indicator{d'=d}+\sum_{b}{\pi_{\jump{Prog}}^b(t,x)\,\indicator{d'=\swap{\deme{EI}}{b}{d}}}\right)\\
    &+\alpha_{\jump{Recov}}(t,x,x')\,\indicator{I>\ellI}\,\indicator{d'=d}+\alpha_{\jump{Wane}}(t,x,x')\,\indicator{d'=d}\\
    &+\sum_{e\in{S_0}}{\indicator{b_e\in\cols{\deme{I}}{d}}\,\indicator{d'=\chop{\deme{I}}{b_e}{d}}\,\delta(e,t)}
    +\sum_{e\in{S_1}}{\indicator{b_e\in\cols{\deme{I}}{d}}\,\indicator{d'=\chop{\deme{II}}{a_eb_e}{d}}\,\delta(e,t)}\\
    &+\sum_{e\in{B}}{\indicator{b_e\cup{b'_e}\in\cols{\deme{I}}{d}}\,\left[\pi_{\jump{Trans}}^{b_eb_e'}(t,x)\,\indicator{d'=\fork{\deme{IIE}}{b_eb_e'}{d}}+\pi_{\jump{Trans}}^{b_e'b_e}(t,x)\,\indicator{d'=\fork{\deme{IIE}}{b_e'b_e}{d}}\right]\,\delta(e,t)}.\\
  \end{split}\\
  \begin{split}
    ...
    &+\indicator{u=\jump{Sample}}\,\sum_{e\in{S_0}}{\alpha_{\jump{Sample}}(t,x,x')\,\indicator{b_e\in\cols{\deme{I}}{d}}\,\left(\tfrac{I'-\ellI'}{I'}\right)\,\indicator{t=e}}\\
    &+\indicator{u=\jump{Sample}}\,\sum_{e\in{S_1}}{\alpha_{\jump{Sample}}(t,x,x')\,\indicator{b_e\in\cols{\deme{I}}{d}}\,\tfrac{1}{I'}\,\indicator{t=e}}\\
    &+\indicator{u=\jump{Trans}}\,\sum_{e\in{B}}{\alpha_{\jump{Trans}}(t,x,x')\,\indicator{b_e\cup{b'_e}\in\cols{\deme{I}}{d}}\,\tfrac{1}{E'I'}\,\left[\tfrac{\Indicator{d'=\fork{\deme{IIE}}{b_eb_e'}{d}}}{\pi_{\jump{Trans}}^{b_eb_e'}(t,x)}+\tfrac{\Indicator{d'=\fork{\deme{IIE}}{b_e'b_e}{d}}}{\pi_{\jump{Trans}}^{b_e'b_e}(t,x)}\right]\,\indicator{t=e}}.\\
  \end{split}\\
  \begin{split}
...
\Cref{fig:seirs-example} shows the results of a numerical calculation based on these choices.
...
Observe that, in this case, according to \cref{eq:seirs-boost}, the boost factor will be $0$, which is the correct penalty to apply.
...
\begin{figure}
  ...
  \caption{
    ...
    Likelihood computation for the SEIRS model by sequential Monte Carlo, using the scheme of \cref{sec:smc}.
...
%%     Likelihood computation for the constant-parameter, linear birth-death-sampling model, according to \cref{thm:obsc-filter} via sequential Monte Carlo.
...
Moreover, inasmuch as the formulae of \cref{thm:obsc-filter} can be efficiently computed via sequential Monte Carlo, explicit expressions for transition probabilities are not needed:
...
If sequential Monte Carlo algorithms are used to compute the likelihoods of \cref{thm:obsc-filter}, then it is straightforward to simultaneously assimilate information from both time-series and genealogical data.
...
These restrictions are not essential, and the proof of \cref{thm:pruned-lik} can be adapted to accommodate relaxations of these assumptions.
This will be developed in a sequel.

The price of the theory's flexibility is primarily computational.
When sequential Monte Carlo is used to evaluate the likelihood in \cref{thm:obsc-filter}, the computational complexity scales linearly with the number of genomic samples.
...
On the other hand, the importance sampling underlying \cref{thm:obsc-filter} will in general require effort that is exponential in the number of demes.
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{0.6\linewidth}l}
    ...
    \hline
    $t\in\Rp$ & time & \cref{sec:notation} \\
    \cadlag & right-continuous with left limits & \cref{sec:notation} \\
    \caglad & left-continuous with right limits & \cref{sec:notation} \\
    $\indicator{P},\Indicator{P}$ & indicator ($1$ if $P$ is true, $0$ if $P$ is false) & \cref{sec:notation} \\
    $\Xrt_t,\Yrt_t,\vt(t,x)$, etc. & left-limits of $\Xr_t,\Yr_t,v(t,x)$, etc. & \cref{sec:notation} \\
    $\rightlim{F},\rightlim{C}$, etc. & right-limits of $F,C$, etc. & \cref{sec:notation} \\
    $\Xspace$ & state-space of the population process & \cref{sec:population-process} \\
    $\Xr_t,\X_t$ & population process & \cref{sec:population-process} \\
    $\Xrh_k,\Xh_k$, $k\in\Zp$ & embedded chain of the population process & \cref{sec:notation} \\
    $\alpha(t,x,x')$ & hazard of jump from $\Xr=x$ to $\Xr=x'$ & \cref{sec:population-process} \\
    $p_0(x)$ & probability density of $\Xr_0$ & \cref{sec:population-process} \\
    KFE & Kolmogorov forward equation & \cref{sec:kolmogorov-eqns} \\
    KBE & Kolmogorov backward equation & \cref{sec:kolmogorov-eqns} \\
    $\delta(t,t')$ & one-sided Dirac delta function & \cref{sec:deltas} \\
    $\Jumps$ & collection of jump marks & \cref{sec:jump-marks} \\
    $\Ur_t,\U_t$ & jump-mark process & \cref{sec:jump-marks} \\
    $\Demes$ & collection of demes & \cref{sec:demes} \\
    $\Hr_t,\H_t$ & history process & \cref{sec:history-process} \\
    $\leaves$ & set of labels of samples or extant lineages & \cref{sec:genealogy} \\
    $\partit(\leaves)$ & collection of partitions of subsets of $\leaves$ & \cref{sec:genealogy} \\
    $\Gr_t,\G_t$ & genealogy process & \cref{sec:genealogy,sec:genealogy-process} \\
    $T, t(G)$ & genealogy time & \cref{sec:genealogy} \\
    $\Zr_t,\Z_t$ & tree-structure process & \cref{sec:genealogy} \\
    $\Yr_t,\Y_t$ & coloring process & \cref{sec:genealogy} \\
    $\fY(Z)$ & set of colorings compatible with tree-structure $Z$ & \cref{sec:genealogy} \\
    $\fY_t(Z)$ & set of colorings compatible with tree-structure $Z$ at time $t$ & \cref{sec:genealogy} \\
    $\col{Y}$ & color (location within the set of demes) & \cref{sec:genealogy} \\
    $\ctr{Y}$  & event indicator & \cref{sec:genealogy} \\
    $\G^{\lab{Z}}$ & tree-structure of a genealogy & \cref{sec:genealogy} \\
    $\G^{\lab{Y}}$ & coloring of a genealogy & \cref{sec:genealogy} \\
    $n$ & deme-occupancy function & \cref{sec:demes} \\
    $r^u$ & production of a jump of mark $u\in\Jumps$ & \cref{sec:genealogy} \\
    $\ell$ & lineage count function & \cref{sec:ells} \\
    $s$ & saturation function & \cref{sec:ells} \\
    ...
    $\tbinratio{n}{\ell}{r}{s}$ & binomial ratio & \cref{sec:binomial-ratio} \\[1ex]
    $\cols{i}{y}$ & set of branches of a given color & \cref{sec:swapnchop} \\
    $\swap{ij}{b}{},\backswap{ij}{b}{}$ & swap operators (associated with inline nodes) & \cref{sec:swapnchop} \\
    $\chop{ij}{ab}{},\backchop{ij}{ab}{}$ & chop operators (associated with samples) & \cref{sec:swapnchop} \\
    $\fork{ijk}{bb'}{},\backfork{ijk}{bb'}{}$ & fork operators (associated with branch points) & \cref{sec:swapnchop} \\
    \hline\hline
  \end{tabular}
\end{table}
...
Similarly, if any event of $\H$ is incompatible with the local structure of $\P$ in the sense of \cref{sec:compatibility}, then necessarily $\CondProb{\Pr_\T=\P}{\Hr_\T=\H}=0$.
...
The $j$-th lineage extends backward in time until it either coalesces with $\Ph_{j-1}$ or reaches $t=0$ (\cref{fig:embedded-chain}).
...
\begin{figure}[b]
  ...
  \caption{
    ...
    \Cref{thm:pruned-lik} is derived by computing the probability that it coalesces at each possible point, conditional on the history of the population process.
...
\end{equation*}
the latter being the binomial ratio defined in \cref{sec:binomial-ratio}.
...
In $\P$, at time $t$, there are $\ell_i=\ell_i(\Yt_t)$ lineages in deme $i$, of which $s_i=s_i(\Y_t,\Yt_t)$ are emergent.
...
The likelihoods that appear in \cref{thm:pruned-filter,thm:obsc-filter} are integrals over large sets of histories.
...
\Cref{eq:adjfilt1,eq:adjfilt2} constitute the \emph{adjoint form} of \cref{eq:gen-filt2,eq:gen-filt3}.
...
This is facilitated by the following Lemma, the statement of which uses a one-sided Dirac delta function (\cref{sec:deltas}).
...
If we view $\Vr_t$ as a weight, then \cref{lemma:monte-carlo} tells us how the $\Vr_t$-weighted average of $\Xr_t$ evolves in time:
this average is simply $\int{w(t,x)\,\dd{x}}$.
Thus, \cref{lemma:monte-carlo} gives a recipe for the integration of \cref{eq:two-part-filter-reg,eq:two-part-filter-sing} in the Monte Carlo sense.
A pseudocode representation of this procedure is given in \cref{alg:smc}.
For simplicity, \cref{alg:smc} is written for the time-homogeneous case, in which neither $\beta(t,x)$ nor $\lambda(t,x)$ depend explicitly on $t$.
...
Our purpose here is merely to illustrate the implications of \cref{lemma:monte-carlo} by showing how the features of \cref{eq:filter-eq-defn-reg,eq:filter-eq-defn-sing} translate into computations, in the simplest possible context.
...
\begin{algorithm}[H]
  \caption{
    ...
    As in \cref{lemma:monte-carlo}, we let
    \begin{equation*}
      \begin{gathered}
