...
\begin{abstract}
  ...
  We then derive exact expressions for the likelihood of an observed genealogy in terms of filter equations, which can be integrated using standard Monte Carlo methods.
...
\begin{table}
  \caption{
    ...
    The filter equation contains $n$ terms of each type.
