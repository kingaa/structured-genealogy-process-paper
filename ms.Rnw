...
We further assume that $\alpha(t,x,x')$ is \cadlag\ as a function of time for all $x,x'\in{\Xspace}$.
...
If only $m$ samples are taken simultaneously at a single time, $T$, the filter equation thus reads
\begin{mathsize}{10pt}{11pt}
  \begin{equation}
    ...
    \begin{gathered}
      ...
      -\mu\,w(t),\quad t\notin{S},
      \qquad
      w(t) = \mu\,\frac{\wt(t)}{\tbinom{n}{2}},\quad t\in{S}.
    \end{gathered}
  \end{equation}
\end{mathsize}%
...
\end{equation}
where $k$ is the number of branch-points and the $s^{}_{i}\coloneq\int{\Indicator{\ell(t)=i}\,\dd{t}}$ are the durations of the \emph{coalescent intervals}, \ie intervals between successive branch-points.
