...
stopifnot(packageVersion("phylopomp")>="0.10.2")
...
A genealogy includes three distinct kinds of nodes:
\begin{inparaenum}[(i)]
...
We let $\event{\G}{T}$ denote the set of all internal and sample node-times of the genealogy $\G_T$;
...
The tree structure of $\G_T$ is defined by a \cadlag\ map $\z:[0,T]\to\func{Z}$ that is monotone in the sense that $t_1\le{t_2}$ implies $\z_{t_1}\preceq{\z_{t_2}}$.
...
let $m_{t}(a)$ be the number of internal or sample nodes encountered as one descends along lineage $a$ from time $t=t(a)$ to $t=0$.
...
Let $\func{Y}(\z)$ denote the set of such maps compatible with $\z$.
...
Note also that a pruned genealogy $\Pr_t$ is characterized by functions $\Pr_t^\y$ and $\Pr_t^\z$ just as an un-pruned genealogy is.
Finally, observe that, since it contains within itself all of its past history, the pruned genealogy process $\Pr_t$ is Markov.
...
Accordingly, suppose $\P_T=(\y,\z)$ is a pruned genealogy and suppose $t\in[0,T]$.
...
Therefore, we can define $\ell$ as a function such that, whenever $\Pr_T=(\yr,\zr)$ is a pruned genealogy, $\ell(\yr_t,\zr_t)$ is the vector of deme-specific lineage counts at time $t$.
...
In particular, given a node time $t$ in a pruned genealogy $\P_T=(\y,\z)$, the number $s_i$ of lineages of deme $i$ emerging from all nodes with time $t$ is well defined and we can write $s=\left(s_i\right)_{i\in\Demes}$.
...
Thus, we can define the \emph{saturation} function such that, whenever $\Pr_T=(\yr,\zr)$ is a pruned genealogy, $s(\yrt_t,\yr_t,\zrt_t,\zr_t)$ is the integer vector of deme-specific numbers of emerging lineages at time $t$.
...
there is a feasible genealogy, $\G_T=(\y,\z)$, and history, $\H_T$,
...
In particular, if $\Pr_T=(\y,\z)$ is a pruned genealogy, we write $\obs(\Pr_T)=\Pr_T^{\z}=\z$ to denote the obscured genealogy.
...
\begin{thm}\label{thm:pruned_lik}
  Suppose $\P^*=(\y^*,\z^*)$ is a given pruned genealogy with $t(\P^*)=T$.
  Define
  ...
  Then
  \begin{equation*}
    \CondProb{\Pr_T=\P^*}{\Hr_T=\H_T}=\Indicator{\event{\H}{T}\supseteq{\event{\P}{}^*}}\,\prod_{t\in\event{\H}{T}}{\phi_{\U_t}(\X_t,\yt^*_t,\y^*_t,\zt^*_t,\z^*_t)}.
  \end{equation*}
\end{thm}
\begin{proof}
  If $\event{\Hr}{T}\nsupseteq\event{\P}{}^*$, then $\Hr_T$ and $\P^*$ are incompatible and $\CondProb{\Pr_T=\P^*}{\Hr_T}=0$.
  Similarly, if any event of $\Hr_T$ is incompatible with the local structure of $\P^*$ in the sense of \cref{sec:compatibility}, then $\CondProb{\Pr_T=\P^*}{\Hr_T}=0$.
  Let us therefore suppose that neither of these conditions hold.
  Conditional on $\Hr_T=\H_T$, at each time $t\in\event{\H}{T}$, a jump of mark $\U_t$ occured, with a production of $r^{\U_t}=(r_i)_{i\in\Demes}$, resulting in a deme-occupancy of $n(\X_t)=(n_i)_{i\in\Demes}$.
  ...
  \end{equation*}
  Returning to the possibility that $\H_T$ is incompatible with $\P^*$, since $\Prob{\Pr_T=\P^*}=0$ if either any $Q_u=0$ or $\event{\P}{}^*\nsupseteq\event{\H}{T}$, we obtain the result.
\end{proof}
...
\begin{thm}
  Suppose that $\P^*=(y^*,z^*)$ is a pruned genealogy with $T=t(\P^*)>0$.
...
\begin{thm}\label{thm:obsc_lik}
  \AAK{[This can be rephrased somewhat more compactly into a result claiming the existence of the Markov process $\y$.]}
  ...
  and that $q(x,y)>0$ whenever $\CondProb{\y_0=y}{\Xr_0=x}>0$.
  ...
  \begin{equation*}
    ...
    \Indicator{\event{\H}{T}\supseteq\event{\z}{}^*}\,\Expect{\frac{1}{q(X_0,\yr_0)}\,\prod_{t\in\event{\H}{T}}{\frac{\phi_{\U_t}(\X_t,\yrt_t,\yr_t,\zt^*_t,\z^*_t)}{\pi_{\U_t}(t,\Xt_t,\X_t,\yrt_t,\yr_t)}}},
  \end{equation*}
...
\begin{proof}
  ...
  Conditional on $\Hr_T$, the initial distribution $q$ and probability kernel $\pi$ generate a Markov chain, $\yrh_k$ such that
  \begin{equation*}
    \begin{gathered}
      \CondProb{\yrh_0}{\Hr_T=\H_T}=q(\X_0,\yrh_0),
      \qquad
  ...
  This construction of $\yr_t$ obviously guarantees that $\event{\H}{t}\supseteq\event{\y}{}\supseteq\event{\z}{}^*$ and that $(\Xr_t,\Ur_t,\yr_t)$ is Markov.

  Let us define $\P(\yr)=(\yr,z^*)$.
  Then, by construction, for every $\yr\in\func{Y}(\z)$, $\obs(\P(\yr))=z^*$ and, conversely, for every pruned genealogy $\Pr$ satisfying $\obs(\Pr)=z^*$, $\Pr^\y$ is a sample path of this process having nonzero probability.
  ...
  Here, by definition,
  \begin{equation*}
    \pi(\yr\vert\H_T)=q(\X_0,\yr_0)\,\prod_{t\in\event{\H}{T}}{\pi_{\U_{t}}(t,\Xt_{t},\X_{t},\yrt_{t},\yr_{t})}.
  \end{equation*}
...
\begin{thm}
  \AAK{[This can be rephrased into a claim about the existence of the kernels $\pi$ and $q$.]}
  Let $z^*$, be a given obscured genealogy.
  Let $\pi$, $q$, and $\yr_t$ be as in \cref{thm:obsc_lik}.
  For $t\in\Rp$, $x,x'\in\Xspace$, $y,y'\in\Yspace^{z^*}_t$, $u\in\Jumps$, define
  \begin{equation*}
    \begin{gathered}
      \beta_u(t,x,x',y,y')=\alpha_u(t,x,x')\,\pi_u(t,x,x',y,y'),\\[2ex]
