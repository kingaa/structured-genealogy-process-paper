...
Thiis computation provides the exact likelihood for a genealogy resulting from a particular, constant population-size, dynamic model \citep[the Moran model, \eg][]{Moran1958,Kingman1982b,Moehle2000}.
...
%% \ELI{[For the claim of generality, should we give a disclaimer that model complexity is limited in practice by both the limited amount of information in data to fit the model, and the numerical tractability of the integration.
%%     Some idea of the largest size which has been tested to date (say, 5 compartments and 1000 sequences?) would be helpful to the reader.
%%     Readers may naturally be skeptical about the practical scope of the methods.
%%     For example, familiar Monte Carlo methods such as BEAST have theoretical guarantees going far beyond their practical applicability.]}
