...
\end{equation*}
for $\xi\in\Xspace$, $\eta,\eta'\in\Yspace$, and $u\in\Jumps$.
Recall that $n:\Xspace\to\Zp$ is the deme-occupancy function, $r^u$ is the production of a jump of mark $u$, $\ell$ is the lineage-count function, and $s$ is the saturation.

\begin{thm}\label{thm:pruned_lik}
  Let $P^*$ be a given pruned genealogy, $t=T(P^*)$, and $y^*=Y^{P^*}$.
  Then
  \begin{equation*}
    \Prob{P^*\vert\Hist_t}=\Indicator{\event(P^*)\subseteq{\event(\Hist_t)}}\,\times\,\prod_{e\in\event(\Hist_t)}{\phi_{U_e}\Big(X_{e},\yt^*(e),y^*(e)\Big)}.
  \end{equation*}
\end{thm}
\begin{proof}
...
\begin{thm}\label{thm:obsc_lik}
  ...
  Suppose moreover that $\pi_u(t,\xi,\xi',\eta,\eta')>0$ for all $t,\xi,\xi'$ whenever $Q_u(\eta,\eta')=1$.
  Let $\rdm{y}_t$ be the Markov jump process generated by the kernel $\pi$, with jump times fixed at $\event(\Hist_t)$.
  Then
  \begin{equation*}
    \Prob{V^*\vert\Hist_t}=\Expect{\prod_{e\in\event(\Hist_t)}{\frac{\phi_{U_e}(X_e,\leftlim{\rdm{y}}_t,\rdm{y}_t)}{\pi_{U_e}(e,\leftlim{X}_e,X_e,\leftlim{\rdm{y}}_e,\rdm{y}_e)}}},
  \end{equation*}
