...
\begin{table}[h!]
  ...
  \begin{tabular}{cclc}
    \hline\hline
    $u$ & $\alpha_u$ & Event type & $r_u$ \\
    \hline
    $\lab{Trans}$  & $\frac{\beta S I}{N}$ \bigstrut & pure birth  & 2 \\
    $\lab{Recov}$  & $\gamma I$        \bigstrut & pure death  & 0 \\
    $\lab{Wane}$   & $\omega R$        \bigstrut & neutral     & 0 \\
    $\lab{Sample}$ & $\psi I$          \bigstrut & pure sample & 1 \\
    \hline\hline
  \end{tabular}
\end{table}
...
\begin{table}[h!]
  ...
  \begin{tabular}{cclc}
    ...
    $u$ & $\alpha_u$ & Event type & $r_u$ \\
    \hline
    $\lab{Trans}$  & $\frac{\beta S I}{N}$ \bigstrut & pure birth     & $(1,1)$ \\
    $\lab{Prog}$   & $\sigma E$        \bigstrut & pure migration & $(0,1)$ \\
    $\lab{Recov}$  & $\gamma I$        \bigstrut & pure death     & $(0,0)$ \\
    $\lab{Wane}$   & $\omega R$        \bigstrut & neutral        & $(0,0)$ \\
    $\lab{Sample}$ & $\psi I$          \bigstrut & pure sample    & $(0,1)$ \\
    \hline\hline
  \end{tabular}
\end{table}
...
\begin{table}[h!]
  ...
  \begin{tabular}{c c c c c c c | c}
    \hline\hline
    & $u$ & $s$ & $\BinRatio{n}{\ell}{r}{s}$ & $y\mapsto{y'}$ & $\pi$ & $Q$ & Line\\[2ex]
    \hline
    \multirow[c]{10}{*}[-1ex]{\rotatebox{90}{$t\notin{{B}\cup{S_0}\cup{S_1}}$}}
    & \multirow{4}{*}{$\lab{Trans}$}
    & $(0,0)$ & $\left(\frac{E-\ell_E}{E}\right)\left(\frac{I-\ell_I}{I}\right)$ \bigstrut & \unbroken \bigstrut & $\frac{I-\ell_I}{I}$ & $1$ & 1 \\
    \cline{3-8}
    && $(1,0)$ & $\frac{I-\ell_I}{E I}$ \bigstrut & \maizeblue \bigstrut & $\frac{1}{2 I}$ & $1$ & 2 \\
    \cline{3-8}
    && $(0,1)$ & $\frac{E-\ell_E}{E I}$ \bigstrut & \maizemaize \bigstrut & $\frac{1}{2 I}$ & $1$ & 3 \\
    \cline{3-8}
    && $(1,1)$ & --- & --- & 0 & $0$ & 4 \\
    \cline{2-8}
    &\multirow{2}{*}{$\lab{Prog}$}
    & $(0,0)$ & $\frac{I-\ell_I}{I}\,\Indicator{E\ge\ell_E}$ \bigstrut & \unbroken \bigstrut & $\frac{E-\ell_E}{E}\,\Indicator{E>\ell_E}$ \bigstrut & $1$ & 5 \\
    \cline{3-8}
    && $(0,1)$ & $\frac{1}{I}\,\Indicator{E\ge\ell_E}$ \bigstrut & \bluemaize \bigstrut & $\frac{1}{E}\,\Indicator{E>\ell_E}$ \bigstrut & $1$ & 6 \\
    \cline{2-8}
    & $\lab{Recov}$ & $(0,0)$ & $\Indicator{I\ge\ell_I}$ \bigstrut & \unbroken \bigstrut & $\Indicator{I>\ell_I}$ \bigstrut & $1$ & 7 \\
    \cline{2-8}
    & $\lab{Wane}$ & $(0,0)$ & $1$ \bigstrut & \unbroken \bigstrut & $1$ & $1$ & 8 \\
    \cline{2-8}
    ...
    & \multirow{4}{*}{$\lab{Trans}$}
    & \multirow{4}{*}{$(1,1)$}
    & $\frac{1}{E I}$ \bigstrut & \branchup \bigstrut & $\frac{1}{2}$ \bigstrut & $1$ & 10 \\
    \cline{4-8}
    &&& $\frac{1}{E I}$ \bigstrut & \branchdown \bigstrut & $\frac{1}{2}$ \bigstrut & $1$ & 11 \\
    \cline{4-8}
    &&& --- \bigstrut & \branchupnix \bigstrut & 0 & $0$ & 12 \\
    \cline{4-8}
    &&& --- \bigstrut & \branchdownnix \bigstrut & 0 & $0$ & 13 \\
    \cline{2-8}
    & $\ast$ & $\ast$ & --- \bigstrut & --- & 0 & $0$ & 14 \\
    \hline
    \multirow[c]{2}{*}[-2ex]{\rotatebox{90}{$t\in{S_0}$} \bigstrut}
    & \multirow{2}{*}{$\lab{Sample}$}
    & \multirow{2}{*}{$(0,0)$}
    & $\frac{I-\ell_I}{I}$ \bigstrut & \samplezero & $1$ & $1$ & 15 \\
    \cline{4-8}
    &&& --- & \samplezeronix \bigstrut & 0 & 0 & 16 \\
    \cline{2-8}
    & $\ast$ & $\ast$ & --- & --- & 0 & $0$ & 19 \\
    \hline
    ...
    & \multirow{2}{*}{$\lab{Sample}$}
    & \multirow{2}{*}{$(0,1)$}
    & $\frac{1}{I}$ \bigstrut & \sampleone \bigstrut & $1$ & $1$ & 17 \\
    \cline{4-8}
    &&& --- & \sampleonenix \bigstrut & 0 & 0 & 18 \\
    \cline{2-8}
    & $\ast$ & $\ast$ & --- & --- & 0 & $0$ & 19 \\
    \hline\hline
  \end{tabular}
...
\begin{mathsize}{9pt}{10pt}
  \begin{equation*}
    \begin{split}
      ...
      &\frac{\beta\,(S+1)\,I}{N}\,w(t,S+1,E-1,I,R)
      -\frac{\beta\,S\,I}{N}\,w(t,S,E,I,R)\\
...
params |>
  with(
    mcap(logLik,sigma,span=0.25)
  ) -> mcap
@

\begin{figure}
  \begin{center}
    <<seirs3_plot,fig.dim=c(8,2.8),out.width="100%">>=
    plot_grid(
      A=seirs_tree |>
...
\begin{table}[h!]
  ...
  \begin{tabular}{cclc}
    ...
    $u$ & $\alpha_u$ & Event type & $r_u$ \\
    \hline
    $\lab{Birth}$  & $\lambda N$  & pure birth  & 2 \\
    $\lab{Death}$  & $\gamma N$   & pure death  & 0 \\
    $\lab{Sample}$ & $\psi N$     & pure sample & 1 \\
    \hline\hline
  \end{tabular}
\end{table}
