...
\begin{defn}
  ...
  \begin{equation}
    \fontsize{9pt}{11pt}\selectfont
    \label[pluralequation]{eq:filter-eq-defn}
    \begin{aligned}
      \frac{\partial{w}}{\partial{t}}(t,x)&=
  ...
  The first of \cref{eq:filter-eq-defn} is the \emph{regular part} of the filter equation;
  the second is known as the \emph{singular part}.
\end{defn}
...
\begin{proof}
  ...
  Assuming that it holds for $|s^{}_t|<K$, one has only to verify the singular part of \cref{eq:two-part-filter}.
  One can integrate \cref{eq:Hdens} directly to accomplish this.
\end{proof}
