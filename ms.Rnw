...
Specifically, we will describe how, at each jump in the population process, a corresponding change occurs in the genealogy, according to lineages branch, die, or move between demes and whether samples are taken.
...
\begin{thm}
  ...
  Then the likelihood of $\z^*_T$ is
  \begin{equation*}
    \lik(\z^*_T)=\sum_y{\int{w(T,x,y)\,\dd{x}}}.
  \end{equation*}
\end{thm}
