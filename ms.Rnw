...
Relations to lookdown construction of \citet{Etheridge2019}.
...
If we postulate that, at time $s_i$, an event occurred at which $\Xr_t$ jumped from state $x$ to state $x'$ with probability $\pi(s_i,x,x')$, the KFE for the process becomes
\begin{align}
  \label{eq:kfe-reg}
  \frac{\partial{v}}{\partial{t}}(t,x)
  &=\int\!{v(t,x')\,\alpha(t,x',x)\,\dd{x'}}
  ...
  \label{eq:kfe-sing}
  v(t,x)
  &=\int\!{\leftlim{v}(t,x')\,\pi(t,x',x)\,\dd{x'}},
  &t\in{S}.
\end{align}
...
\begin{equation*}
  \alpha(t,x,x')\mapsto\alpha(t,x,x')+\sum_{s\in{S}}{\delta(t,s)\,\pi(t,x,x')},
\end{equation*}
...
In particular, we suppose that there are a countable set of subpopulations, within each of which individual lineages are statistically identical.
...
\Cref{appendix:examples} works out the theory for each of these examples.
...
The Kingman coalescent is another workhorse in existing phylodynamic approaches.
...
In the classical \citet{Kingman1982a} case, $m$ samples are taken simultaneously at a single time, $T$.
For this case, if $B$ is the set of branch-times and $\ell(t)$ is the number of lineages in the genealogy at time $t$, the filter equation reads
\begin{mathsize}{10pt}{11pt}
  \begin{equation}
...
More generally, if in addition samples are taken through time at a per-capita rate $\psi(t)$ such that the set of sample-times in the genealogy is $S=S_0\cup{S_1}$, where $S_0$, $S_1$ are the sets of times of terminal and inline samples, respectively, then the filter equation reads
\begin{mathsize}{10pt}{11pt}
  \begin{equation}
    ...
    \begin{gathered}
      ...
      w(t) = \psi(t)\,\left(1-\frac{\ell(t)}{n}\right)\,\wt(t),\quad t\in{S_0},
      \qquad
      w(t) = \frac{\psi(t)}{n}\,\wt(t),\quad t\in{S_1}.
    \end{gathered}
  \end{equation}
\end{mathsize}%
...
\begin{equation}
  ...
  +\sum_{t\in{S_0}}\log{\left[\psi(t)\,\left(1-\frac{\ell(t)}{n}\right)\right]}
  +\sum_{t\in{S_1}}\log{\left[\frac{\psi(t)}{n}\right]}
