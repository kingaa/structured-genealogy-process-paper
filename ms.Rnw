...
The above may be compactly summarized by stating that if $u(t,x)$ satisfies the Kolmogorov forward equation (KFE),
\begin{equation}
  \label{eq:kfe}
  \frac{\partial{u}}{\partial{t}}(t,x)
  =\int\!{u(t,x')\,\alpha(t,x',x)\,\dd{x'}}
  -\int\!{u(t,x)\,\alpha(t,x,x')\,\dd{x'}},
\end{equation}
...
\Cref{lemma:monte-carlo} shows how this can be computed via Sequential Monte Carlo.
