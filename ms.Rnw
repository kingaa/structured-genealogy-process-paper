...
In particular, let $\leaves$ be a countable set of labels---referring to samples and/or extant lineages---and let $\partit(\leaves)$ be the set of all collections of nonempty, finite, mutually-disjoint subsets of $\leaves$.
...
In particular, $\col{Y}\circ{Z}:[0,T]\to\Demes$ can be taken to be \cadlag\ and $\ctr{Y}\circ{Z}:[0,T]\to\Set{0,1}$ has finite support.
...
\begin{remark}
  ...
  The quantity $w(t,x)$ appearing in \cref{eq:pruned-filter-eqn} is not itself a probability density, except at $t=T$.
  Rather, it represents an expectation of partial versions of the products appearing in \cref{thm:pruned-lik} (containing only the factors for times $<t$) over population trajectories having terminal value $x$ at time $t$.
\end{remark}
...
\begin{figure}
  ...
  \caption{
    ...
    (A)~A~simulated genealogy for $\beta=\Sexpr{sirs_params$Beta}$, $\gamma=\Sexpr{sirs_params$gamma}$, $\omega=\Sexpr{sirs_params$omega}$, $\psi=\Sexpr{sirs_params$psi}$, $(S_0,I_0,R_0)=(\Sexpr{with(sirs_params,c(S0,I0,R0)*pop)})$.
...
\begin{table}[H]
  \caption{
    ...
    \textbf{Bold face} is used to distinguish random elements.
    \label{tab:symbols}
  }
  \vspace{1ex}
  \begin{tabular}{lp{0.6\linewidth}l}
    ...
    $\indicator{}$ & indicator & \cref{sec:notation} \\
    $\Xrt_t,\Yrt_t,\vt(t,x)$, etc. & left-limits of $\Xr_t,\Yr_t,v(t,x)$, etc. & \cref{sec:notation} \\
    $\rightlim{F},\rightlim{C}$, etc. & right-limits of $\Yr_t,F,C$, etc. & \cref{sec:notation} \\
    ...
    $\leaves$ & set of labels & \cref{sec:genealogy} \\
    $\partit(\leaves)$ & set of partitions of labels & \cref{sec:genealogy} \\
    ...
    $\col{Y}$ & color & \cref{sec:genealogy} \\
    ...
    $Q$ & compatibility indicator function & \cref{sec:compatibility} \\
    $\tbinratio{n}{\ell}{r}{s}$ & binomial ratio & \cref{sec:binomial-ratio} \\[1ex]
    $\cols{i}{y}$ & set of branches of a given deme & \cref{sec:swapnchop} \\
    $\swap{ij}{b}{},\backswap{ij}{b}{}$ & swap operators & \cref{sec:swapnchop} \\
    $\chop{ij}{ab}{},\backchop{ij}{ab}{}$ & chop operators & \cref{sec:swapnchop} \\
    $\fork{ijk}{bb'}{},\backfork{ijk}{bb'}{}$ & fork operators & \cref{sec:swapnchop} \\
    \hline\hline
  \end{tabular}
\end{table}
