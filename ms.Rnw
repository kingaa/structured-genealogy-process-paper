...
Infective demes are interchangeable and we define the migration function from Deme $i$ to $j$ for any $i\neq j$ and $i,j=1,\dots,J$ as $M_{ij}:\Uspace\to\N$ to represent the number of infectives migrating to Deme $j$ from Deme $i$.
Thus the set of migration is $\Mset_{ij}\coloneqq\,M^{-1}_{ij}(\{1\})$ since we assume single migration at a single migration event. 
...
\begin{equation}
  \jumprate_u(t,x) > 0 \Longrightarrow I^j(x+u)-I^j(x)=\sum_{i}\,B_{ij}(u)+\sum_{i}M_{ij}(u)-D_j(u)-\sum_{i}M_{ji}(u),
\end{equation}
...
\begin{equation*}
  \begin{aligned}
    ...
    \Masterset^j(\Gen) &\coloneqq , \\
    \Cset^{j,j'}(\Gen) &\coloneqq , \\
    \Lset^j(\Gen) &\coloneqq , \\
    \Tset^j(\Gen) &\coloneqq , \\
    \Zset^{j,j'}(\Gen) &\coloneqq , \\
...
\begin{enumerate}[(a)]
  ...
  \item There was a migration event, that is, $u_k\in\Mset^{j,j'}$ within $\halfopen{a_q,s_q}$, given that it was not presented in the pre-existing genealogy $\Och_{q-1}$, \ie $t_k\notin\Zset^{j,j'}(\Och_{q-1})$: 
    \begin{enumerate}
      \item if this migration event exactly involved the $q$-th sample and was presented as a deme-transition event in $\Och_q$, \ie $t_k\in\Zset^{j,j'}(\Och_q)$, then $\phi_{qk}^{j''}=\delta_{j',j''}/\left((I_{k-1}^{j}-\ell_{q,k-1}^{j})(I_{k}^{j'}-\ell_{qk}^{j'})\right)$;
      \item otherwise, $t_k\notin\Zset^{j,j'}(\Och_q)$, then $\phi_{qk}^{j''}=1-\delta_{j',j''}/\left((I_{k-1}^{j}-\ell_{q,k-1}^{j})(I_{k}^{j'}-\ell_{qk}^{j'})\right)$.
    \end{enumerate} 
...
\begin{equation}\label{eq:qcases}
  ...
  \begin{cases}
    ...
    1-\frac{\delta_{j,j''}}{I_k^{j}-\ell_{qk}^{j}}, &\text{if}\ t_k\in\Zset^{j,j'}(\Och_{q-1}),\ t_k\in\Zset^{j,j'}(\Och_q),\ u_k\in\Bset^{j,j'},\ \text{and}\ j\neq j',\\[12pt]
    % not in Z
    ...
    1-\frac{\delta_{j',j''}}{I_{k}^{j'}-\ell_{qk}^{j'}}, &\text{if}\ t_k\notin\Zset^{j,j'}(\Och_{q-1}),\ t_k\in(a_q,s_q),\ u_k\in\Bset^{j,j'},\ \text{and}\ j\neq j',\\[12pt]
...
\begin{thm}\label{thm:prunehthm1}
  With the definitions as above, $P_{\Och_i|\Hist}(o|h) = \prod_{q=1}^{i}\prod_{k=1}^{K}\left(\sum_{j''=1}^{J}\phi_{qk}^{j''}\right)=\prod_{q=1}^{i}\prod_{k=1}^{K}\phi_{qk}^{j''_q}$, where $j''_q$ is the deme of the $q$-th sample lineage.
\end{thm}
...
\begin{proof}
  ...
  Last but not least, suppose $u_k\in\Bset^{j,j'}$.
