...
\begin{equation*}
  \Zrt_t\colonequals
  \begin{cases}
    \displaystyle\lim_{s\,\uparrow\,{t}}\;\Zr_{s}, & t>0,\\[2ex]
    \Zr_0, & t=0.\\
  \end{cases}
\end{equation*}
...
\begin{equation*}
  ...
  \end{gathered}
\end{equation*}
We will further assume that $\Xr_t$ is non-explosive, so that $\Prob{\Kr_t<\infty}=1$ for all $t$.
...
\AAK{[Maybe it would be better to speak instead of $\Hr_t$ being the random $\sigma$-algebra generated by the sequences $(\Trh_k,\Xrh_k,\Urh_k)_{k=1}^{\Kr_t}$.
    The only thing we do with $\Hr$ is condition on it.
    One (perhaps minor) problem with this is that we cannot then speak about the measure on histories.
]}
...
If $\H$ is such a history, we define $\time(\H)$ to be the right endpoint of its domain and use the notation $\event{\H}=\Set{\Th_1,\dots,\Th_{\K_t}}\subset{[0,\time(\H)]}$ to denote the set of its jump times.
...
\begin{figure}
  ...
  \caption{
    $\time(\G)=10$.
    Two demes, $\Demes=\Set{\func{blue},\func{yellow}}$.
    Tip nodes, denoting extant lineages, are shown in black;
    sample nodes are shown in blue;
    internal nodes are indicated using green dots.
    Note that internal nodes occur both at branch-points and inline.
    Wherever a lineage moves from one deme (color) to another, an internal node occurs, but not conversely.
    \label{fig:geneal}
  }
\end{figure}

A \emph{genealogy}, $\G$, encapsulates the relationships of shared ancestry among a set of lineages that are extant at some time $\time(\G)\in\Rp$, and perhaps a set of samples collected at earlier times (\cref{fig:geneal}A).
...
Accordingly, we can visualize a genealogy as a tree, the nodes and edges of which are painted with a distinct color for each deme (\cref{fig:geneal}A).
...
Formally, we define a genealogy, $\G$, to be an ordered pair $(\y,\z)$.
The $\z$ element specifies the genealogy's tree structure, as follows.
Let $\leaves$ be a countable set of labels and let $\func{Z}$ be the set of all partitions of subsets of $\leaves$.
Partition \emph{fineness} defines a partial order on $\func{Z}$.
Specifically, for $\zeta,\zeta'\in{\func{Z}}$, we say $\zeta\preceq{\zeta'}$ if and only if for every $b'\in{\zeta'}$ there is $b\in{\zeta}$ such that $b\supseteq{b'}$.
If $\time(\G)=T$, the tree structure of $\G$ is defined by a \cadlag\ map $\z:[0,T]\to\func{Z}$ that is monotone in the sense that $t_1\le{t_2}$ implies $\z_{t_1}\preceq{\z_{t_2}}$.
An element $b\in\z_t$ is a set of labels;
it represents the branch of the tree that bears the corresponding lineages.
We use the notation $\event{\z}$ to denote the set of all event-times in the tree $\z$.
...
The $\y$ element of $\G$ specifies the coloring of branches and locations of tip, sample, and internal nodes (including inline nodes).
In particular, if $a$ is the label of any tip or sample node and $t\in[0,T]$, we define $d_{t}(a)$ to be the deme in which the lineage of $a$ was found at time $t$ and think of this as the color of the corresponding branch at that time.
We will also need to count the number of nodes along each lineage:
let $m_{t}(a)$ be the number of internal or sample nodes encountered as one descends along lineage $a$ from time $t=\time(a)$ to $t=0$.
Observe that $m_t(a)$ is a simple counting process, with $m_0(a)=0$ for all $a$.
Formally, we define the coloring component $\y$ as follows.
Let $\Yspace=\Demes\times\Zp$ and write $\upsilon\in\Yspace$ component-wise as $\upsilon=(\upsilon^d,\upsilon^m)$.
For any $\zeta\in\func{Z}$, an element $\eta\in\Yspace^{\zeta}$ is a map from the partition $\zeta$ into $\Yspace$.
Now, suppose we are given a tree $\z:[0,T]\to\func{Z}$ and a \cadlag\ map $\y:[0,T]\to{\coprod_{t\in[0,T]}{\Yspace^{\z_t}}}$.
Let $\event{\y}$ denote the set of discontinuities of $\y$.
We say that $\y$ is \emph{compatible} with $\z$ if it is a valid coloring of the tree $\z$.
Formally, compatibility requires the following:
\begin{inparaenum}[(i)]
\item
  $\event{\y}\supseteq\event{\z}$;
\item
  for every $t\in[0,T]$, $\y_t\in{\Yspace^{\z_t}}$;
\item
  if $0\le{t_1}\le{t_2}\le{T}$, $b_1\in\z_{t_1}$, $b_2\in\z_{t_2}$, and $b_1\supseteq{b_2}$,
  then $y^m_{t_1}\le{y^m_{t_2}}$;
\item $y^m_{t_1}=y^m_{t_2}$ implies $y^d_{t_1}=y^d_{t_2}$.
\end{inparaenum}
Let $\func{Y}(\z)$ denote the set of such maps compatible with $\z$ and $\func{Y}_t(\z)\coloneq\CondSet{y_t}{y\in\func{Y}(\z)}$.
\AAK{[These conditions are necessary but not sufficient.]}
\AAK{[The coproduct is a fiber bundle over the $\z$ and $\func{Y}(\z)$ is a section of this bundle.]}

The two \cadlag\ functions $\y$ and $\z$ specify a genealogy completely and uniquely.
When the genealogy is not clear from context, we write $\G^{\y}$ and $\G^{\z}$ to refer to the respective components of genealogy $\G$.
...
It is very important to note that, conditional on $\Urh_k$, the number of parents and number of offspring in each deme is determined and the random choice of which lineages die, migrate, are sampled, or sire offspring is independent of the choices at any other times and independent of $(\Trh_k,\Xrh_k)$ for all $k$.
...
Note also that a pruned genealogy $\P$ is characterized by functions $\P^\y$ and $\P^\z$ just as an un-pruned genealogy is.
...
Accordingly, suppose $\P=(\y,\z)$ is a pruned genealogy with $\time(\P)=T$ and suppose $t\in[0,T]$.
...
Therefore, we can define $\ell$ as a function such that, whenever $\P=(\y,\z)$ is a pruned genealogy, $\ell(\y_t,\z_t)$ is the vector of deme-specific lineage counts at time $t$.
...
In particular, given a node time $t$ in a pruned genealogy $\P=(\y,\z)$, the number $s_i$ of lineages of deme $i$ emerging from all nodes with time $t$ is well defined and we can write $s=\left(s_i\right)_{i\in\Demes}$.
...
Thus, we can define the \emph{saturation} function such that, whenever $\P=(\y,\z)$ is a pruned genealogy, $s(\yrt_t,\yr_t,\zrt_t,\zr_t)$ is the integer vector of deme-specific numbers of emerging lineages at time $t$.
...
That is, $Q_u(\eta,\eta',\zeta,\zeta')=1$ if and only if
there is a feasible genealogy, $\G=(\y,\z)$, and history, $\H$,
and a $t\in{[0,T]}$ such that,
given $\Gr_T=\G$ and $\Hr_T=\H$,
$\U_t=u$,
$\yt_t=\eta$, $\y_t=\eta'$,
$\zt_t=\zeta$, and $\z_t=\zeta'$.
...
In particular, if $\P=(\y,\z)$ is a pruned genealogy, we write $\obs(\P)=\P^{\z}=\z$ to denote the obscured genealogy.
...
\begin{thm}\label{thm:pruned_lik}
  Suppose $\P^*=(\y^*,\z^*)$ is a given pruned genealogy with $\time(\P^*)=T$.
  Define
  ...
  Then
  \begin{equation*}
    \CondProb{\Pr_T=\P^*}{\Hr_T=\H}=\Indicator{\event{\H}{}\supseteq{\event{\P}{}^*}}\,\prod_{t\in\event{\H}{}}{\phi_{\U_t}(\X_t,\yt^*_t,\y^*_t,\zt^*_t,\z^*_t)}.
  \end{equation*}
\end{thm}
\begin{proof}
  If $\event{\H}{}\nsupseteq\event{\P}{}^*$, then $\H$ and $\P^*$ are incompatible and $\CondProb{\Pr_T=\P^*}{\Hr_T=\H}=0$.
  Similarly, if any event of $\H$ is incompatible with the local structure of $\P^*$ in the sense of \cref{sec:compatibility}, then $\CondProb{\Pr_T=\P^*}{\Hr_T=\H}=0$.
  Let us therefore suppose that neither of these conditions hold.
  Conditional on $\Hr_T=\H$, at each time $t\in\event{\H}{}$, a jump of mark $\U_t$ occured, with a production of $r^{\U_t}=(r_i)_{i\in\Demes}$, resulting in a deme-occupancy of $n(\X_t)=(n_i)_{i\in\Demes}$.
  In $\P^*$, at time $t$, there are $\ell_i=\ell_i(\y^*_t,\z^*_t)$ lineages in deme $i$, of which $s_i=s_i(\yt^*_t,\y^*_t,\zt^*_t,\z^*_t)$ are emergent.
  ...
  Moreover, since, again conditional on $\Hr_T=\H$, the identities of the lineages involved in a genealogical event are random and independent of the identities selected at all other events, we have established that
  \begin{equation*}
    \CondProb{\Pr_T=\P^*}{\Hr_T=\H}=\prod_{t\in\event{\H}{}}{\BinRatio{n(\X_t)}{\ell(\y^*_t,\z^*_t)}{r^{\U_t}}{s(\yt^*_t,\y^*_t,\zt^*_t,\z^*_t)}}.
  \end{equation*}
  Returning to the possibility that $\H$ is incompatible with $\P^*$, since $\Prob{\Pr_T=\P^*}=0$ if either any $Q_u=0$ or $\event{\P}{}^*\nsubseteq\event{\H}{}$, we obtain the result.
\end{proof}
...
\begin{thm}
  Suppose that $\P^*=(\y^*,\z^*)$ is a pruned genealogy with $\time(\P^*)=T>0$.
  ...
  \begin{equation}
    \begin{aligned}
      ...
      &t\notin{\event{\P}{}^*},\\
      w(t,x)=&\sum_u{\int{\wt(t,x')\,\alpha_u(t,x',x)\,\phi_u(x,\yt^*_t,\y^*_t,\zt^*_t,\z^*_t)\,\dd{x'}}},
      &t\in{\event{\P}{}^*},
    \end{aligned}
  \end{equation}
...
\begin{thm}\label{thm:obsc_lik}
  Suppose $\z^*$ is a given obscured genealogy with $\time(\z^*)=T$.
  Let $q$ and $\pi$ be probability kernels, such that for all $u\in\Jumps$, $t\in\Rp$, $x,x'\in\Xspace$, $y,y'\in\func{Y}_t(\z^*)$,
  \begin{equation*}
    \begin{gathered}
      q(x,y)\ge{0},\qquad
      \sum_{y\in\func{Y}_0(\z^*)}{q(x,y)}=1, \qquad
      \pi_u(t,x,x',y,y')\ge{0},\qquad
      \sum_{y'\in\func{Y}_t(\z^*)}{\pi_u(t,x,x',y,y')}=1.
    \end{gathered}
  \end{equation*}
  Suppose moreover that $\pi_u(t,x,x',y,y')>0$ whenever $\alpha_u(t,x,x')\,Q_u(y,y',\zt^*_t,\z^*_t)>0$
  and that $q(x,y)>0$ whenever $\CondProb{\Pr_0^\y=y}{\Xr_0=x}>0$.
  Then there is a stochastic jump process $\yr_t$ with sample paths in $\func{Y}(\z^*)$ such that $(\Xr_t,\Ur_t,\yr_t)$ is Markov and
  \begin{equation*}
    \CondProb{\Pr^{\z}_T=\z^*}{\Hr_T=\H}=
    \Indicator{\event{\H}{}\supseteq\event{\z}{}^*}\,\Expect{\frac{1}{q(X_0,\yr_0)}\,\prod_{t\in\event{\H}{}}{\frac{\phi_{\U_t}(\X_t,\yrt_t,\yr_t,\zt^*_t,\z^*_t)}{\pi_{\U_t}(t,\Xt_t,\X_t,\yrt_t,\yr_t)}}},
  \end{equation*}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:IS1}
    \CondProb{\zr_T=\z^*}{\Hr_T=\H}=\CondExpect{\Indicator{\Pr^{\z}_T=\z^*}}{\Hr_T=\H}.
  \end{equation}
  Our strategy will be to evaluate \cref{eq:IS1} using importance sampling:
  we will propose pruned genealogies compatible with $\z^*_t$ as sample paths from a stochastic process driven by $\Xr_t$ and
  evaluate the the expectation in \cref{eq:IS1} by summing over these paths.
  Conditional on $\Hr_T=\H$, the initial distribution $q$ and probability kernel $\pi$ generate a Markov chain, $\yrh_k$ such that
  \begin{equation*}
    \begin{gathered}
      \CondProb{\yrh_0}{\Hr_T=\H}=q(\X_0,\yrh_0),
      \qquad
      \CondProb{\yrh_k}{\yrh_{k-1},\Hr_T=\H}=\pi_{\Uh_k}(\Th_k,\Xh_{k-1},\Xh_{k},\yrh_{k-1},\yrh_k).
    \end{gathered}
  \end{equation*}
  The required process $\yr_t$ is the unique \cadlag\ process with event times $\Trh_k$ and $\yrh_k$ as its embedded chain.
  This construction of $\yr_t$ obviously guarantees that $\event{\H}{}\supseteq\event{\y}{}\supseteq\event{\z}{}^*$ and that $(\Xr_t,\Ur_t,\yr_t)$ is Markov.

  Now, for $\y\in\func{Y}(\z^*)$, let us define $C(\y)=(\y,\z^*)$.
  Then, by construction, $\obs(C(\y))=\z^*$ and,
  conversely, for every pruned genealogy $\P$ satisfying $\P^{\z}=\z^*$, $C(\P^{\y})=\P$.
  Moreover, the conditions on the kernels $q$ and $\pi$ guarantee that, if $\CondProb{\Pr_T=\P}{\Hr_T=\H}>0$ and $\P^{\z}=\z^*$, then $\Prob{\yr=\P^{\y}}>0$.
  We therefore have that
  \begin{equation*}
    \CondProb{\zr_T=\z^*}{\Hr_T=\H}=
    \Expect{\frac{\CondProb{\Pr_T=C(\yr)}{\Hr_T=\H}}{\pi(\yr\vert\H)}},
  \end{equation*}
...
\begin{thm}
  Let $\z^*$ be a given obscured genealogy and $T=\time(\z^*)$.
  ...
  \begin{equation*}
    \begin{gathered}
      ...
      \psi_u(t,x,x',y,y')=\frac{\phi_u(x',y,y',\zt^*_t,\z^*_t)}{\pi_u(t,x,x',y,y')},
    \end{gathered}
  \end{equation*}
  and if $w=w(t,x,y)$ satisfies
  the initial condition $w(0,x,y)=p_0(x)\,q(x,y)$
...
Allowable to borrow information from the future.
...
\begin{lemma}
  ...
  \begin{equation*}
    \CondExpect{\Vr_t}{\Hr_t=\H}=\prod_{e\in\event{\H}{t}}{B(e,\Xt_e,\X_e)},
  \end{equation*}
