...
Conditional on $\Hist_t$, both $\X_t$ and $\U_t$ are deterministic as are the embedded chain, $(\rdm{\hat{X}}_k,\rdm{\hat{U}}_k)$, and the point process of event times $\rdm{\hat{T}}_k$.
(Recall that $\rdm{\hat{X}}_k=\rdm{X}_{\rdm{\hat{T}}_k}$ and $\rdm{\hat{U}}_k=\rdm{U}_{\rdm{\hat{T}}_k}$.)
...
Note that the lineages that die as a result of an event do not count in the production.
...
\item \emph{branch nodes}, which represent ancestral events;
\item \emph{sample nodes}, which represent labeled samples.
\end{inparaenum}
More formally, we can take a genealogy, $G$, to be a finite sequence of nodes, together with a time.
...
\begin{enumerate}[(a)]
\item A birth-type event at time $t$ results in the creation of one new branch node for each parent lineage.
...
\item In a death-type event, all the lineages $j$ that die are removed.
  Branch nodes without children are then recursively removed.
  Sample nodes are never removed.
\item In a migration-type event, one branch node is added for each migrating lineage;
...
Given a genealogy $\Gen_t$, one obtains the \emph{pruned genealogy}, $\Prune_t=\prune(\Gen_t)$ by first dropping every tip node and then recursively dropping every branch node without children.
In a pruned genealogy only branch and sample nodes remain, and sample nodes are found at all of the leaves and some of the interior nodes of the genealogy.
...
\item Let $\anc^{P}:\Zp\times\Rp\to\Zp$ be such that $\anc^{P}_j(t)$ indicates the unique lineage, ancestral of lineage $j$, and alive at time $t$.
  ...
  \ELI{[How is this defined when there is immigration in the system?]}
...
Note in particular that, $Y^{P_t}_j(t')\ne\leftlim{Y}^{P_t}_j(t')$ if and only if $t'$ is the time of an event from which lineage $j$ emerges.
...
\end{equation*}
With this definition, for any $P$, if $y=Y^P$, then $s_i(\yt_t,y_t)$ is the number of lineages in deme $i$ that emerge from an event at time $t$ and $s(\yt_t,y_t)$ is the non-negative integer vector telling how many lineages in each deme emerge at time $t$.
In particular, $s(y,y)=0$ so that if $e\notin\event{P}{t}$, then $s(\yt_e,y_e)=0$.
We refer to $s(\yt_t,y_t)$ as the \emph{saturation} at time $t$.
...
\paragraph{Likelihood for pruned genealogies}
...
Let us define the function $Q:\Jumps\times\Yspace^2\to\{0,1\}$ so that $Q_u(y,y')=1$ if and only if a change $y\to{y'}$ in a pruned genealogy is compatible with an event of mark $u$ at that time.
...
\begin{thm}\label{thm:pruned_lik}
  ...
  Then
  \begin{equation*}
    \Prob{P^*_t\vert\Hist_t}=\prod_{e\in\event{\Hist}{t}}{\BinRatio{n(X_e)}{\ell(y^*_e)}{r^{U_e}}{s(\yt^*_e,y^*_e)}\,Q_{U_e}(\yt^*_e,y^*_e)}
    \:\times\:\Indicator{\event{P^*}{t}\subseteq{\event{\Hist}{t}}}.
  \end{equation*}
  \ELI{[Do we need $Q$, if the binomial ratio is defined to be zero in impossible cases?]}
\end{thm}
\begin{proof}
  ...
  In $P^*_t$, at time $e$, there are $\ell_i=\ell_i(y^*_t)$ lineages in deme $i$, of which $s_i=s_i(\yt^*_t,y^*_t)$ are emergent.
  ...
  \begin{equation*}
    \Prob{P^*_t\vert\Hist_t=H_t}=\prod_{e\in\event{H}{t}}{\BinRatio{n(X_e)}{\ell(y^*_e)}{r^{U_e}}{s(\yt^*_e,y^*_e)}}.
  \end{equation*}
...
\paragraph{Likelihood for obscured genealogies}
...
\begin{thm}\label{thm:obsc_lik}
  ...
  \begin{equation*}
    \begin{aligned}
      \Prob{V^*_t\vert\Hist_t=H_t}=&\Expect{\left.\prod_{e\in\event{H}{t}}{\BinRatio{n(X_e)}{\ell(\rdm{y}_e)}{r^{U_e}}{s(\leftlim{\rdm{y}}_e,\rdm{y}_e)}\,\frac{\scriptsize\kappa_{V^*_t}(e,\leftlim{\rdm{y}}_e,\rdm{y}_e)}{\scriptsize\pi_{U_e}(e,\leftlim{X}_e,X_e,\leftlim{\rdm{y}}_e,\rdm{y}_e)}}\;\right\vert\;\Hist_t}\\
      &\qquad\times\Indicator{\event{V^*}{t}\subseteq\event{H}{t}},
    \end{aligned}
  \end{equation*}
...
\begin{proof}
  ...
  \end{equation*}
  Now let $\event{H}{t}=\{t_1,t_2,\dots,t_K\}$.
  $\rdm{\hat{y}}_k$ is the embedded chain of a unique, piecewise-constant, \cadlag\ process $\rdm{y}_t$, with jump times at $t_k$.
  This construction of $\rdm{y}_t$ obviously guarantees that $\event{P(\rdm{y})}{t}\subseteq\event{H}{t}$.
  ...
  Finally, observe that the condition $\obs(P(\rdm{y}))=V$ can be factorized for all obscured genealogies $V$.
  In particular, there is a function $\kappa_{V}:[0,t]\times\Yspace^2\to\{0,1\}$ such that $\kappa_{V}(s,\eta,\eta')=1$ if and only if
  there is $y:\Rp\to\Yspace$ such that $\obs(P(y))=V$, $\leftlim{y}_s=\eta$, and $y_s=\eta'$, for all $s\in[0,T(V)]$.
...
\begin{thm}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      &\ +\sum_{e\in\event{V^*}{T}}{\delta(t,e)\,\left\{\sum_{uy'}{\int{w(t,x')\,\beta_u(t,x',x)\,\phi_u(t,x,y',y)\,\dd{x'}}}-w(t,x)\right\}},
    \end{aligned}
  \end{equation}
  with the initial condition $w(0,x,y)=p_0(x)$.
  Then the likelihood of $V^*_T$ is
  \begin{equation*}
    \lik(V^*_T)=\sum_y{\int{w(T,x,y)\,\dd{x}}}.
  \end{equation*}
  \ELI{[I was surprised by the assertion of existence of $k_V$ here, and then the proof providing a specific construction---why not assert the exact form?
      The proof is also unusual---via looking for an importance sampling solution.]}
\end{thm}
...
\begin{defn}
  ...
  \begin{equation}\label{eq:kfe2}
    ...
    -\int{w(t,x)\,\beta(t,x,x')\,\dd{x'}}.
  \end{equation}
  Suppose that $B(t,x,x')\ge{0}$ and $\lambda(t,x)\in\R$ are given measurable functions.
  \ELI{[Ranges are defined elementwise, which is not the usual math formalism,
      e.g., $\lambda:T\times\Xspace\to\mathbb{R}$
      or just say $\lambda(t,x)$ takes values in $\mathbb{R}$.]}
  ...
  \begin{equation}\label{eq:filtereq}
    ...
    \end{aligned}
  \end{equation}
  is the \emph{filter equation} with \emph{driver} $\X_t$ (or \emph{generated by} $\beta$), \emph{boost} $B$, \emph{decay} $\lambda$, and \emph{observed events} $\mathcal{M}$.
  \ELI{[Why is $B=1$, $\lambda=0$ not equal to Eq.~(A1)?
      (It is if we have $|M|=0$.)]}
  \AAK{[The KFE is a special case of a filter equation.]}
\end{defn}
...
\begin{lemma}\label{lemma:filtereq}
  ...
  \ELI{[Note: the last term in eq (A3) is not in the form of $-lambda\,u$ for some $\lambda$.
      So, Eq (A3) is not a filter equation in the form of Eq (A1).
  ]}
  \AAK{[But it is a KFE, for a process with a drift.]}
\end{lemma}
\begin{proof}
...
\begin{prop}\label{prop:zeroboost}
  ...
  \begin{equation}\label{eq:Vprod}
    \Expect{\rdm{V}_t\;\vert\;\Hist_t=H_t}=\prod_{e\in\event{H}{t}}{B(e,\leftlim{X}_e,X_e)},
  \end{equation}
  ...
  \ELI{[$dw/dt$ for $X$ could be $du/dt$ since this is not the same $w$ as the subsequent line.
      The terminology uses the letter $w$ as a dummy variable, but this makes it harder to talk about the solutions to different KFEs.]}
\end{prop}
\begin{proof}
...
\begin{prop}\label{prop:fullfilt}
  Let $\X_t$, $\Hist_t$, $B$, and $\rdm{V}_t$ be as in \cref{prop:zeroboost}.
  Suppose that $\mathcal{M}\subseteq\halfclosed{0,t}$ is a fixed, finite set of observed event times.
  Let $\Expect{\rdm{V}_t\;\vert\;\mathcal{M}}$ denote the density of $\rdm{V}_t$ conditional on $\mathcal{M}\subseteq\event{\Hist}{t}$.
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  then $\Expect{\rdm{V}_t\;\vert\;\mathcal{M}}=\int{w(t,x)\,\dd{x}}$.
  \ELI{[(a) line 2: density -> expectation.
      ...
      (c) what is a ``fixed, finite set of event times''?
      $M_t^H$ is a random variable, so it cannot usually be fixed.
      $M$ is also treated sometimes as a random variable, even though it is defined to be fixed.
      I see this means, ``conditional on the event that $M_t^H$ contains $M$'', but we should be explicit, to avoid confusion about whether $M$ is a random variable or not.]}
\end{prop}
\begin{proof}
  Suppose $s\in[0,t]$ and let
  \begin{equation*}
    w(s,x)=\Expect{\prod_{e\in\event{\Hist}{s}}{B\left(e,\Xt_e,\X_e\right)}}.
  \end{equation*}
