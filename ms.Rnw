...
\begin{thm}
  Suppose we have a pruned genealogy $P^*_T$, $T>0$, with event times $\event(P^*_T)$.
  Let $y^*=Y^{\Prune^*_T}$.
  For $x,x'\in\Xspace$, $y,y'\in\Yspace$, $u\in\Jumps$, define
  \begin{equation*}
    \begin{gathered}
      \phi_u\big(x,y,y')=\BinRatio{n(x)}{\ell(y')}{r^u}{s(y,y')}\qquad
      \text{and} \qquad
      \beta_u(t,x,x')=\alpha_u(t,x,x')\,Q_u(y,y'),
    \end{gathered}
  \end{equation*}
  where $Q_u(y,y')$ is as in \cref{thm:pruned_lik}.
  Fix $\nu>0$ and suppose $w=w(t,x)$ satisfies the filter equation
  \begin{equation}
    \begin{aligned}
      \frac{\partial{w}}{\partial{t}}=
      &\sum_u{\int{w(t,x')\,\beta_u(t,x',x)\,\phi_u\big(x,\yt^*_t,y^*_t\big)\,\dd{x'}}}
      -\sum_u{\int{w(t,x)\,\beta_u(t,x,x')\,\dd{x'}}}\\
      &\quad+\sum_{e\in\event(P^*_T)}{\delta(t,e)\,\left\{\sum_u{\int{w(t,x')\,\frac{\beta_u(t,x',x)}{\nu}\,\phi_u\big(x,\yt^*_t,y^*_t\big)\,\dd{x'}}}-w(t,x)\right\}},
    \end{aligned}
  \end{equation}
...
\begin{defn}
  ...
  \begin{equation}\label{eq:filtereq}
    \begin{aligned}
      ...
      -\int{w(t,x)\,\beta(t,x,x')\,\dd{x'}}
      -\lambda(t,x)\,w(t,x)\\
      &\quad+\sum_{e\in\mathcal{M}}\left\{\int{w(t,x')\,\frac{\beta(t,x',x)}{\nu}\,B(t,x',x)\,\dd{x'}}-w(t,x)\right\}.
    \end{aligned}
  \end{equation}
...
\begin{prop}\label{prop:zeroboost}
  ...
  \begin{equation*}
    \Expect{\rdm{V}_t\;\vert\;\Hist_t=H_t}=\prod_{e\in\event(H_t)}{Q(e,\leftlim{X}_e,X_e)\,B(e,\leftlim{X}_e,X_e)},
  \end{equation*}
...
However, we also need to compute likelihoods for data that include observed event times.
...
To avoid having to work with such complex objects, it is convenient to represent the likelihood as a density with respect to a constant-rate Poisson point process.
...
The same reasoning shows how to compute expectations of a real-valued process $\rdm{V}_t$ directed by $\X_t$.
...
\begin{prop}\label{prop:fullfilt}
  ...
  \begin{equation*}
    \Expect{\rdm{V}_t\;\vert\;\Hist_t}=\prod_{e\in{\event(\Hist_t)}}{Q(e,\leftlim{X}_e,X_e)\,B(e,\leftlim{X}_e,X_e)}.
  \end{equation*}
...
\begin{remark}
  Suppose $\X_t$, $\Y_t$ have KFEs
  \begin{equation}
    \begin{aligned}
      \frac{\partial{w}}{\partial{t}}(t,x)=
      &\int{w(t,x')\,\alpha(t,x',x)\,\dd{x'}}
      -\int{w(t,x)\,\alpha(t,x,x')\,\dd{x'}},\\
      \frac{\partial{w}}{\partial{t}}(t,y)=
      &\int{w(t,y')\,\beta(t,y',y)\,\dd{y'}}
      -\int{w(t,y)\,\beta(t,y,y')\,\dd{y'}},
    \end{aligned}
  \end{equation}
  where $\beta(t,y,y')=\alpha(t,y',y)\,\left(1+\frac{1}{\nu}\sum_{e\in\mathcal{M}}{\delta(t,e)}\right)$.
\end{remark}
